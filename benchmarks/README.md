...
| `cluster`     | single-node bootstrap, 3-node gossip convergence, pub-sub fan-out, sharded entity round-trip, **node-count scaling** (1 / 2 / 3 / 5 sharded nodes, one subprocess per size), **TCP message cost** (the same actor local vs. on a second node on a worker thread over a real `TcpTransport` on loopback — encode → frame → socket → decode; tell throughput and ask latency, every batch completion-verified) |
| `worker`      | **Bun Worker count scaling** (1 / 2 / 4 / 8 / auto workers running a CPU-bound task) — true multi-core parallelism, unlike a router pool inside one ActorSystem; **mesh message cost** (what an actor message pays to cross a node boundary: local vs. a second node in-thread vs. a real worker thread); **task-offload break-even** (from how much CPU work per call a worker pool beats running the function inline) |
...
| **What a message pays to cross a thread** | [`worker/mesh-message-cost.ts`](worker/mesh-message-cost.ts) | The same actor local, on a second node in the same thread (the protocol cost: envelope, ref codec, broker relay, two clones) and on a real worker thread.  Tell throughput and ask latency, every batch completion-verified.  The ratio is what moving an actor to a thread has to pay back per message (#1566). |
| **What a message pays to cross a socket** | [`cluster/tcp-message-cost.ts`](cluster/tcp-message-cost.ts) | The same actor local and on a second cluster node over a real `TcpTransport` on loopback — the one benchmark that serialises: envelope, tagged-JSON frame, socket, `FrameDecoder`.  The far node runs on a worker thread, deliberately: with both nodes on one event loop the synchronous tell loop fills the loopback buffers faster than the receiver can drain them and the tail of a burst is lost silently (#931); every batch is completion-verified, so that surfaces as a throw rather than a number. |
| **When does offloading a function pay?** | [`worker/task-offload-breakeven.ts`](worker/task-offload-breakeven.ts) | The same CPU loop inline, on one worker (the boundary cost alone) and on a pool sized to the machine, across 1 µs … 10 ms of work per call.  Prints the crossover — the number the offload docs quote (#1558). |
  ...
  await runGroup('single-node · my-bench', [
    {
      ...
      unit: 'actor',
      iterations: 5_000,
      run: () => { const ref = system.spawnAnonymous(() => new Noop()); ref.stop(); },
    },
  ]);
