...
object ResultFile:
  ...
  def write(path: Path, environment: EnvironmentBlock, results: Seq[Harness.ScenarioResult]): Unit =
    ...
    json.name("framework").beginObject()
      ...
      // called "akka" would collide on the published file name.
      // `Main.outputPath()` spells the derived name out — keep them in step.
      .name("name").value("akka-scala")
      ...
      // Akka 2.7 and later are BSL 1.1, not Apache 2.0. It travels with the
      // number because a reader comparing frameworks is usually also choosing
      // one, and this is the kind of fact that decides it.
  ...
  private def akkaVersion(): String =
    val fromManifest = Option(classOf[akka.actor.typed.ActorSystem[?]].getPackage)
    ...
    fromManifest.getOrElse(akka.Version.current)
