...
/**
 * The Pekko (JVM) arm's entry point (#27).
 *
 * <p>Byte-for-byte the ../akka arm apart from the package prefix, and that is
 * deliberate: the two JVM arms differ only in their dependency, so any gap
 * between them is the fork rather than the benchmark.
 *
...
public final class Main {
    ...
    public static void main(String[] args) throws Exception {
        ActorSystem<Actors.GuardianCommand> system =
                ActorSystem.create(Actors.guardian(), "comparison-pekko-java");
        ...
        try {
            ...
            results.add(Harness.measure("spawn", "batch=100", "actor",
                    ...
                    + "termination. Pekko Typed only lets an actor spawn actors, so the batch is "
     ...
     * and nothing complains.  {@code framework.name} is {@code pekko-java} in
    ...
    private static Path outputPath() {
        ...
        return round == null || round.isBlank()
                ? results.resolve("pekko-java-jvm.json")
                : results.resolve(".rounds").resolve("pekko-java-jvm-r" + round + ".json");
    }
}
