 ...
 * `../pekko-java`, driven through its Scala API in the idiomatic functional
...
object Main:
  ...
  def main(args: Array[String]): Unit =
    val system = ActorSystem(Actors.guardian, "comparison-pekko-scala")
   ...
   * `framework.name` is `pekko-scala` in [[ResultFile]] and `runtime.name` is
   * `jvm`, so this reads `pekko-scala-jvm`.
   */
  private def outputPath(): Path =
    ...
    if round == null || round.isBlank then results.resolve("pekko-scala-jvm.json")
    else results.resolve(".rounds").resolve(s"pekko-scala-jvm-r$round.json")
