...
describe('SnapshotStore — master-key rotation', () => {
  test('snapshot written with v0 active is still readable when v1 is now active and v0 retired', async () => {
    ...
    const phase1Store = new ObjectStorageSnapshotStore({ backend, encryption: phase1 });
    ...
    const phase2Store = new ObjectStorageSnapshotStore({ backend, encryption: phase2 });
  ...
  test('reading a body whose version is not in the keyring throws with operator-friendly hint', async () => {
    ...
    const writeStore = new ObjectStorageSnapshotStore({ backend, encryption: v2Config });
...
describe('DurableStateStore — master-key rotation', () => {
  test('upsert under v0 then v1 — both revisions readable post-rotation', async () => {
    ...
    const phase1 = new ObjectStorageDurableStateStore({ backend, encryption: v0Cfg });
    ...
    const phase2 = new ObjectStorageDurableStateStore({ backend, encryption: v1Cfg });
