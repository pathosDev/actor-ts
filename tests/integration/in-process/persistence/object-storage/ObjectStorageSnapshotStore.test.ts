...
describe('ObjectStorageSnapshotStore — save / loadLatest', () => {
  test('save returns a snapshot with the supplied seq + state', async () => {
    const s = new ObjectStorageSnapshotStore(ObjectStorageSnapshotStoreOptions.create().withBackend(backend));
...
describe('ObjectStorageSnapshotStore — keepN pruning', () => {
  test('keeps only the most recent N snapshots after each save', async () => {
    const s = new ObjectStorageSnapshotStore(ObjectStorageSnapshotStoreOptions.create().withBackend(backend).withKeepN(2));
  ...
  test('keepN=0 disables pruning', async () => {
    const s = new ObjectStorageSnapshotStore(ObjectStorageSnapshotStoreOptions.create().withBackend(backend).withKeepN(0));
...
describe('ObjectStorageSnapshotStore — compression resolver', () => {
  ...
  test('round-trip survives gzip and zstd', async () => {
    const s = new ObjectStorageSnapshotStore(
      ObjectStorageSnapshotStoreOptions.create()
        .withBackend(backend)
        .withCompression((pid) => pid.startsWith('zstd-')
          ? { algorithm: 'zstd' }
          : { algorithm: 'gzip' }),
    );
...
describe('ObjectStorageSnapshotStore — prefix', () => {
  test('plugin prefix is prepended to every key', async () => {
    const s = new ObjectStorageSnapshotStore(ObjectStorageSnapshotStoreOptions.create().withBackend(backend).withPrefix('env-prod/'));
  ...
  test('per-tenant resolver: two pids → two distinct subkeys; cross-decryption fails', async () => {
    ...
    const s = new ObjectStorageSnapshotStore(
      ObjectStorageSnapshotStoreOptions.create()
        ...
        .withEncryption((pid) => pid.startsWith('tenant-')
