...
async function write(
  ...
  if (!response.ok) throw new Error(`ddata write on ${host} → ${response.status}: ${await response.text()}`);
  return await response.json() as { elapsedMs: number };
}
...
async function read(
  ...
  if (!response.ok && response.status !== 404) {
    throw new Error(`ddata read on ${host} → ${response.status}: ${await response.text()}`);
  }
  const body = await response.json() as { value?: string | null; elapsedMs: number };
  return { value: body.value ?? null, elapsedMs: body.elapsedMs, status: response.status };
}
...
export const scenario: Scenario = {
  ...
  async run(context) {
    if (context.nodes.length < 3) {
      console.log(`[04] skipping — need >=3 nodes for majority, have ${context.nodes.length}`);
      return;
    }
    const [nodeA, , nodeC, nodeD] = context.nodes;

    // 1. Baseline write from node-a, majority consistency.
    console.log(`[04] writing ${KEY}=initial from ${nodeA} (majority, baseline)...`);
    const baselineWrite = await write(nodeA!, context.controlPort, KEY, 'initial', 'majority');
    ...
    await Promise.all(context.nodes.map(async (h) => {
      await waitFor(
        `${h} reads "initial"`,
        async () => {
          const readResult = await read(h, context.controlPort, KEY, 'majority');
    ...
    const delayCalls = context.nodes.map((h) =>
      fetch(`http://${h}:${context.controlPort}/test/delay?ms=50`, { method: 'POST' }),
    );
    const delayResponses = await Promise.all(delayCalls);
    for (const response of delayResponses) if (!response.ok) throw new Error(`[04] delay install failed: ${response.status}`);
    ...
    try {
      ...
      const stormWrite = await write(nodeC!, context.controlPort, KEY, 'updated', 'majority');
      ...
      await waitFor(
        ...
        async () => {
          const readResult = await read(nodeD!, context.controlPort, KEY, 'majority');
          return readResult.value === 'updated';
        },
        15_000,
        500,
      );
      const stormRead = await read(nodeD!, context.controlPort, KEY, 'majority');
      ...
      const clearCalls = context.nodes.map((h) =>
        fetch(`http://${h}:${context.controlPort}/test/delay?ms=0`, { method: 'POST' }),
      );
