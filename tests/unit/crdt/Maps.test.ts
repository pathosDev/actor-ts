...
describe('LWWMap — typical workload', () => {
  test('per-user options: independent keys merge without contention', () => {
    ...
    const first = LWWMap.empty<string, string>()
      .put('node-a', 'alice/theme', 'dark', 100)
      .put('node-a', 'alice/lang', 'en', 100);
    const second = LWWMap.empty<string, string>()
      .put('node-b', 'bob/theme', 'light', 100)
      .put('node-b', 'bob/lang', 'de', 100);
    const merged = first.merge(second);
    expect(merged.size).toBe(4);
    expect(merged.get('alice/theme')).toBe('dark');
    expect(merged.get('bob/lang')).toBe('de');
  });

  test('feature flag flip: newer write wins, older value disappears', () => {
    const first = LWWMap.empty<string, boolean>()
      .put('a', 'flag.beta', false, 1_000);
    const second = LWWMap.empty<string, boolean>()
      .put('b', 'flag.beta', true, 2_000);
    expect(first.merge(second).get('flag.beta')).toBe(true);
  ...
  test('removed key stays removed under first slow gossip from the original writer', () => {
    ...
    const first = LWWMap.empty<string, string>().put('a', 'k', 'v', 100);
    const second = first.remove('b', 'k', 200);     // tombstone @ 200
    expect(second.has('k')).toBe(false);
    expect(second.merge(first).has('k')).toBe(false); // re-merging the older state
  });
  ...
  test('non-string keys with custom identity', () => {
    ...
    expect(merged.get({ tenant: 't1', id: 1 })).toBe('alice');
...
describe('ORMap — typical workload', () => {
  ...
  test('per-tenant counters: each tenant has its own GCounter', () => {
    ...
    const first = empty
      .update('node-a', 'tenant-1', () => GCounter.empty(), (third) => third.increment('node-a', 5))
      .update('node-a', 'tenant-2', () => GCounter.empty(), (third) => third.increment('node-a', 2));
    const second = empty
      .update('node-b', 'tenant-1', () => GCounter.empty(), (third) => third.increment('node-b', 3));
    const merged = first.merge(second);
    expect(merged.get('tenant-1')!.value()).toBe(8);     // per-replica sum
    expect(merged.get('tenant-2')!.value()).toBe(2);
  });
  ...
  test('JSON round-trip with the DistributedData inner-decoder', () => {
    ...
    const merged = ORMap.empty<string, ORSet<string>>()
  ...
  test('nested mutation via update preserves sender attribution in the inner ORSet', () => {
    ...
    expect(merged.get('k')!.has('item')).toBe(false);
  });
});
...
describe('MVRegister — typical workload', () => {
  ...
  test('three-way concurrent assigns all survive their merge', () => {
    const first = MVRegister.empty<string>().assign('a', 'A');
    const second = MVRegister.empty<string>().assign('b', 'B');
    const third = MVRegister.empty<string>().assign('c', 'C');
...
describe('GCounterMap — typical workload', () => {
  test('per-route hit counter: independent routes accumulate separately', () => {
    const first = GCounterMap.empty<string>()
      .increment('node-a', '/api/users', 12)
      .increment('node-a', '/api/login', 3);
    const second = GCounterMap.empty<string>()
      .increment('node-b', '/api/users', 7)
      .increment('node-b', '/api/orders', 5);
    const merged = first.merge(second);
    expect(merged.value('/api/users')).toBe(19);     // 12 + 7 across replicas
    expect(merged.value('/api/login')).toBe(3);
    expect(merged.value('/api/orders')).toBe(5);
    expect(merged.total()).toBe(27);
    expect(merged.size).toBe(3);
  });

  test('idempotent under repeated gossip from the same peer', () => {
    const first = GCounterMap.empty<string>().increment('a', 'tag-1', 5);
    const second = GCounterMap.empty<string>().increment('b', 'tag-1', 3);
    expect(first.merge(second).merge(second).merge(second).value('tag-1')).toBe(8);
  });
});
