...
const req = (remoteAddress?: string, headers: Record<string, string> = {}): HttpRequest => ({
  method: 'GET',
  path: '/',
  headers,
  query: {},
...
describe('IpAllowlist — IPv4', () => {
  test('allows an IP inside the CIDR', async () => {
    ...
    expect(await mw(req('10.5.6.7'), next)).toBe(okResponse);
  });

  test('rejects an IP outside the CIDR', async () => {
    const mw = IpAllowlist({ allow: ['10.0.0.0/8'] });
    await expect(mw(req('192.168.1.5'), next)).rejects.toThrow(HttpError);
  });

  test('matches the exact host with /32', async () => {
    const mw = IpAllowlist({ allow: ['127.0.0.1/32'] });
    expect(await mw(req('127.0.0.1'), next)).toBe(okResponse);
    await expect(mw(req('127.0.0.2'), next)).rejects.toThrow(HttpError);
  });

  test('handles a /16 prefix correctly', async () => {
    const mw = IpAllowlist({ allow: ['172.16.0.0/16'] });
    expect(await mw(req('172.16.0.1'), next)).toBe(okResponse);
    expect(await mw(req('172.16.255.254'), next)).toBe(okResponse);
    await expect(mw(req('172.17.0.1'), next)).rejects.toThrow(HttpError);
  });
...
describe('IpAllowlist — IPv6', () => {
  test('matches IPv6 loopback /128', async () => {
    ...
    expect(await mw(req('::1'), next)).toBe(okResponse);
    await expect(mw(req('::2'), next)).rejects.toThrow(HttpError);
  });

  test('matches a /64 prefix', async () => {
    const mw = IpAllowlist({ allow: ['2001:db8::/32'] });
    expect(await mw(req('2001:db8:1::1'), next)).toBe(okResponse);
    expect(await mw(req('2001:db8:ffff:ffff::'), next)).toBe(okResponse);
    await expect(mw(req('2001:db9::1'), next)).rejects.toThrow(HttpError);
  });
  ...
  test('handles IPv4-mapped IPv6 (::ffff:a.b.c.d) against an IPv4 CIDR', async () => {
    ...
    expect(await mw(req('::ffff:10.5.6.7'), next)).toBe(okResponse);
    await expect(mw(req('::ffff:192.168.1.5'), next)).rejects.toThrow(HttpError);
  });
});
...
describe('IpAllowlist — fail-closed', () => {
  test('rejects request with no remoteAddress (fail-secure)', async () => {
    const mw = IpAllowlist({ allow: ['10.0.0.0/8'] });
    await expect(mw(req(undefined), next)).rejects.toThrow(/no client address/);
  });
  ...
  test('honours custom getClientIp extractor for x-forwarded-for', async () => {
    ...
    expect(await mw(req(undefined, { 'x-forwarded-for': '10.1.2.3' }), next)).toBe(okResponse);
    await expect(mw(req(undefined, { 'x-forwarded-for': '192.168.1.1' }), next)).rejects.toThrow(HttpError);
  });
  ...
  test('unparseable peer address fails closed (does not match any CIDR)', async () => {
    const mw = IpAllowlist({ allow: ['10.0.0.0/8'] });
    await expect(mw(req('not.an.ip'), next)).rejects.toThrow(HttpError);
  });
});
