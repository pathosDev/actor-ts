...
describe('requestId', () => {
  test('generates an id, forwards it to the handler, and echoes it on the response', async () => {
    ...
    const res = await requestId()(req(), async (enriched) => {
  ...
  test('accepts a well-formed incoming id', async () => {
    const res = await requestId()(req({ 'x-request-id': 'trace-abc_123' }), async () => ({ status: Status.OK, body: 'x' }));
    expect(res.headers?.['x-request-id']).toBe('trace-abc_123');
  });

  test('replaces a hostile incoming id', async () => {
    const hostile = 'x\r\nSet-Cookie: y=z';
    const res = await requestId()(req({ 'x-request-id': hostile }), async () => ({ status: Status.OK, body: 'x' }));
  ...
  test('trustIncoming=false always generates', async () => {
    ...
    const res = await mw(req({ 'x-request-id': 'client-supplied' }), async () => ({ status: Status.OK, body: 'x' }));
    expect(res.headers?.['x-request-id']).toBe('fixed');
  });

  test('does not overwrite a handler-set id header', async () => {
    const res: HttpResponse = await requestId()(req(), async () => ({ status: Status.OK, body: 'x', headers: { 'x-request-id': 'handler' } }));
    expect(res.headers?.['x-request-id']).toBe('handler');
  });
});
