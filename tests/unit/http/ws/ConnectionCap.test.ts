...
import { DEFAULT_WS_POLICY } from '../../../../src/http/ws/WsPolicy.js';
import { jsonCodec } from '../../../../src/http/ws/WsCodec.js';
import type { WebSocketSocketAdapter } from '../../../../src/http/ws/SocketAdapter.js';
...
describe('wireConnection — maxConnections admission cap (WS-5)', () => {
  test('rejects connections beyond the cap with 1013, admits the rest', () => {
    ...
    const policy = { ...DEFAULT_WS_POLICY, maxConnections: 2 };
  ...
  test('separate hubs (routes) have independent counts', () => {
    ...
    const policy = { ...DEFAULT_WS_POLICY, maxConnections: 1 };
    const codec = jsonCodec() as never;
  ...
  test('default policy (Infinity) admits everything', () => {
    ...
    const codec = jsonCodec() as never;
    for (let i = 0; i < 50; i++) {
      wireConnection({} as never, hub, req, fakeSocket().socket, codec, DEFAULT_WS_POLICY);
    }
