...
import { writeRawHttpResponse } from '../../../../src/http/websocket/rawResponse.js';
...
describe('writeRawHttpResponse', () => {
  test('emits a well-formed response and destroys the socket', () => {
    ...
    writeRawHttpResponse(fake.sock, { status: 404, body: 'Not Found' });
    expect(fake.data.startsWith('HTTP/1.1 404 Not Found\r\n')).toBe(true);
    expect(fake.data.endsWith('\r\n\r\nNot Found')).toBe(true);
