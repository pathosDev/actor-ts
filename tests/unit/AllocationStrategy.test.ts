...
describe('HashAllocationStrategy.allocate', () => {
  test('is deterministic regardless of candidate order', () => {
    const s = new HashAllocationStrategy();
    for (let sh = 0; sh < 32; sh++) {
      const a = s.allocate(sh, [n1, n2, n3], new Map());
      const b = s.allocate(sh, [n3, n1, n2], new Map());
  ...
  test('throws on empty candidates', () => {
    const s = new HashAllocationStrategy();
    expect(() => s.allocate(0, [], new Map())).toThrow(/no candidates/);
  });
});
...
describe('HashAllocationStrategy.rebalance', () => {
  test('returns empty when ownership matches the hash mapping', () => {
    ...
    expect(Array.from(s.rebalance(current, [n1, n2, n3], new Set()))).toEqual([]);
  });
  ...
  test('flags shards whose current owner differs from the hash mapping', () => {
    ...
    const out = s.rebalance(current, [n1, n2, n3], new Set());
  ...
  test('skips shards already being rebalanced', () => {
    ...
    const out = s.rebalance(current, [n1, n2, n3], new Set([0]));
...
describe('LeastShardAllocationStrategy.allocate', () => {
  test('picks an empty candidate when one exists', () => {
    const s = new LeastShardAllocationStrategy();
    const current = loadsOf([
      [n1, [0, 1, 2]],
      [n2, []],
      [n3, [3]],
    ]);
    expect(s.allocate(9, [n1, n2, n3], current).equals(n2)).toBe(true);
  });
  ...
  test('breaks ties by address order', () => {
    ...
    expect(s.allocate(9, [n1, n2, n3], current).equals(n1)).toBe(true);
  });
...
describe('LeastShardAllocationStrategy.rebalance', () => {
  test('returns empty when load spread is below threshold', () => {
    const s = new LeastShardAllocationStrategy(5, 10);
    const current = loadsOf([
      [n1, [0, 1, 2]],
      [n2, [3, 4]],
    ]);
    expect(s.rebalance(current, [n1, n2], new Set()).size).toBe(0);
  });

  test('returns shards from the busiest candidate when spread ≥ threshold', () => {
    const s = new LeastShardAllocationStrategy(1, 10);
    const current = loadsOf([
      [n1, [0, 1, 2, 3]],
      [n2, []],
    ]);
    const out = s.rebalance(current, [n1, n2], new Set());
  ...
  test('respects maxSimultaneousRebalance', () => {
    const s = new LeastShardAllocationStrategy(1, 2);
    const current = loadsOf([
      [n1, [0, 1, 2, 3, 4]],
      [n2, []],
    ]);
  ...
  test('skips shards already being rebalanced', () => {
    ...
    const out = s.rebalance(current, [n1, n2], new Set([0, 1]));
    expect(out.has(0)).toBe(false);
    expect(out.has(1)).toBe(false);
  });
  ...
  test('returns empty when only one candidate exists', () => {
    ...
    expect(s.rebalance(current, [n1], new Set()).size).toBe(0);
  });
});
