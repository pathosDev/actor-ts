  ...
  test('returns parsed NodeAddresses', async () => {
    const p = new ConfigSeedProvider(
      ConfigSeedProviderOptions.create()
        .withSeeds(['sys@h1:1000', 'h2:2000']).withSystemName('sys'),
    );
...
describe('DnsSeedProvider', () => {
  test('resolves A records via injected function', async () => {
    const p = new DnsSeedProvider(
      DnsSeedProviderOptions.create()
        .withHostname('fake.local').withSystemName('sys').withPort(2552)
  ...
  test('SRV mode takes ports from the records', async () => {
    const p = new DnsSeedProvider(
      DnsSeedProviderOptions.create()
        .withHostname('_actor-ts._tcp.fake.local').withSystemName('sys').withPort(0)
        .withUseSrv(true)
...
describe('AggregateSeedProvider', () => {
  test('first non-empty provider wins', async () => {
    ...
    const fallback = new ConfigSeedProvider(
      ConfigSeedProviderOptions.create().withSeeds(['sys@h:1000']).withSystemName('sys'),
    );
    const ok = new ConfigSeedProvider(
      ConfigSeedProviderOptions.create().withSeeds(['sys@h:2000']).withSystemName('sys'),
    );
  ...
  test('maps returned pod IPs into NodeAddresses', async () => {
    const p = new KubernetesApiSeedProvider(
      KubernetesApiSeedProviderOptions.create()
        .withNamespace('default').withServiceName('cluster-app').withSystemName('sys').withPort(2552)
