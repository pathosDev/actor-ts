...
export const description = 'the __bidirectionalmultimap__ tag survives a built-package serializer round-trip';

export async function run({ actorTs }) {
  const { BidirectionalMultiMap, JsonSerializer } = actorTs;
  ...
  if (restored.size !== 3) throw new Error(`size ${restored.size} !== 3`);
  if (!restored.has('news', 'ada')) throw new Error('forward direction lost');
  // Never written to the wire — if this answers, it was genuinely rebuilt.
  const holders = [...restored.getKeys('ada')].join(',');
  if (holders !== 'news,sport') throw new Error(`inverse not rebuilt: ${holders}`);
  ...
  // Participant pruning is the invariant the class exists for; it must hold on
  // the built bundle too, including after a round-trip.
  restored.deleteRight('ada');
  if (restored.hasLeft('sport')) {
    throw new Error('sport held only ada and should have been pruned with it');
  }
  if (restored.size !== 1 || [...restored.rights()].join(',') !== 'grace') {
    throw new Error(`pruning left the relation inconsistent: size=${restored.size}`);
  }
}
