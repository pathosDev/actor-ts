...
See [Marshalling](/actor-ts/http/marshalling/) for the full body
story.
...
- **[Marshalling](/actor-ts/http/marshalling/)** — `entity<T>(req)`
  decoding, response encoding via Accept.
- **[Backends — Fastify](/actor-ts/http/backends/fastify/)** —
  the default backend's options.

The [`Route` API reference](/actor-ts/api/types/route/) covers the
full type surface.
