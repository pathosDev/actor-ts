...
// Or: coordinatedShutdown.installProcessHooks(['SIGTERM', 'SIGINT', 'SIGUSR2']);
```

This attaches handlers that call `coordinatedShutdown.run(new ProcessTerminateReason(signal))`.
...
When SIGTERM arrives, the hook fires `coordinatedShutdown.run(...)`, the pipeline
