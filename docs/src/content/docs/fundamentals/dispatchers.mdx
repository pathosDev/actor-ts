...
- **[Parallelism](/fundamentals/parallelism/)** — a dispatcher decides
  *when* an actor runs on its thread, never *which* thread; actual
  parallelism is actors on worker threads, from one config number.
- **[Blocking and CPU-bound work](/fundamentals/blocking-and-cpu-bound-work/)** —
  no dispatcher makes a `*Sync(` call or a hashing loop cheaper; where that
  work goes instead.
