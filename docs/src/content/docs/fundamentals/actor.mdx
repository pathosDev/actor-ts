...
- **The type parameter `<Cmd>`** constrains what messages this actor
  ...
  call site.  See [Messages](/actor-ts/fundamentals/messages/) for the
...
choice into one value.  See [Props](/actor-ts/fundamentals/props/) for
the full surface.
...
schedule.  For request/response, see [Ask pattern](/actor-ts/fundamentals/ask-pattern/).
...
<Aside type="tip" title="Restart semantics">
  ...
  [Supervision](/actor-ts/fundamentals/supervision/) for the full
  decision tree.
</Aside>
...
See [Actor paths](/actor-ts/fundamentals/actor-paths/) for the path
hierarchy and [Supervision](/actor-ts/fundamentals/supervision/) for
...
- **[Messages](/actor-ts/fundamentals/messages/)** — the discriminated-
  union convention every actor uses for its incoming type.
- **[Actor paths](/actor-ts/fundamentals/actor-paths/)** — how
...
The [`Actor` class API reference](/actor-ts/api/classes/actor/) has the
