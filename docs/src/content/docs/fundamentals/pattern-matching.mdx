...
description: The `match(msg).with(...).exhaustive()` idiom that actor-ts uses everywhere — built on ts-pattern. Compile-time exhaustiveness, type narrowing, and the conventions for matching on `kind` discriminators.
---
...
2. **Type narrowing inside each arm.**  No `as` casts, no manual
   `if (msg.kind === ...)` guards inside handlers.
...
so `cmd.to` is a `number` with no cast.
