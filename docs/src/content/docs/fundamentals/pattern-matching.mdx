...
Inside the `set` arm, `m` is automatically typed as
...
- Akka uses sealed-trait subclasses; in TS-land, `kind` is the
  most common community idiom.
...
- **[Actor](/actor-ts/fundamentals/actor/)** — the `onReceive`
...
- **[Typed actors](/actor-ts/typed/typed-actor/)** — the typed
