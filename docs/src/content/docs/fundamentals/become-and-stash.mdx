...
[FSM](/persistence/fsm/overview/) pattern — explicit state +
...
- **The stash has a capacity.**  The buffer is bounded to prevent
  memory leaks from runaway stashing; overflow throws
  `StashOverflowError`.  The bound is a fixed 1024 messages,
  compiled in — there's no configuration knob for it.
- **A bounded mailbox bounds the replay too.**  Mailboxes are
  unbounded by default and none of this applies to them, but if you
  set one with `withMailboxCapacity(...)`, the replay is subject to
  it: `unstashAll()` onto a full mailbox drops under `drop-head` /
  `drop-new` (shedding the *newest* queued messages to make room for
  the older stashed ones) and throws `MailboxFullError` under
  `reject`, leaving the batch stashed.  See
  [Mailboxes](/fundamentals/mailboxes/#mailboxes--stash).
...
<Aside type="caution" title="Stash + restart = stash lost">
  On `Restart` — and on `Stop` — the actor instance is thrown away,
  including the stash buffer.  The stash cannot carry over: the new
  instance has none of the state that made those messages
  un-handleable in the first place.  They are routed to
  **dead letters**, so the loss is at least visible; if the messages
  matter across failure, persist them (`PersistentActor`) or have a
  parent re-send them once the restart settles.

  The same holds for the typed DSL's
  [`Behaviors.withStash`](/typed/behaviors/) buffer, including the
  restart a `Behaviors.supervise` strategy performs on its own.
</Aside>
...
- **[FSM overview](/persistence/fsm/overview/)** — a more formal
