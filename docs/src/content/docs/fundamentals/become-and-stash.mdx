...
<Aside type="caution" title="Stash + restart = state lost">
  ...
  stash buffer.  Stashed messages are silently dropped, not routed
  to dead letters.  If the messages matter across failure, persist
  them (PersistentActor) or have a parent re-send them after the
  restart settles.
</Aside>
