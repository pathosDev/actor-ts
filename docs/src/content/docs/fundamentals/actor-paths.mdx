...
See [Refs across nodes](/actor-ts/cluster/refs-across-nodes/) for
how the host-aware path is wire-encoded.

## Where to next

- **[Props](/actor-ts/fundamentals/props/)** — the configuration
...
- **[Discovery](/actor-ts/discovery/overview/)** — for actors
  whose location isn't known by path but by service-registry
  semantics.

The [`ActorPath`](/actor-ts/api/classes/actorpath/) and
[`ActorSelection`](/actor-ts/api/classes/actorselection/) API
