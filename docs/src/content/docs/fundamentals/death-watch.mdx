---
title: Death watch
description: How an actor observes when another actor stops — `context.watch(ref)` triggers a `Terminated(ref)` system message in the watcher when `ref` terminates, and `context.watchWith(ref, message)` swaps that signal for a message of the watcher's own protocol. Symmetric to supervision but for sibling and cross-tree observation.
---

Supervision catches *failure* — what to do when a child actor's
`onReceive` throws.  **Death watch** catches *termination* — knowing
that some other actor has stopped, whatever the reason (clean stop,
crash beyond restart limits, parent terminated).

The two mechanisms are different on purpose: a child failing is its
parent's problem to handle; a sibling stopping is a notification
event that any other actor can subscribe to.

## A minimal example

```ts
import { match, P } from 'ts-pattern';
import { Actor, ActorSystem, Terminated, type ActorRef } from 'actor-ts';

type WatchCommand = { kind: 'watch'; ref: ActorRef };
type WatcherMessage = Terminated | WatchCommand;

class Watcher extends Actor<WatcherMessage> {
  override onReceive(message: WatcherMessage): void {
    match(message)
      .with(P.instanceOf(Terminated), (t) => this.onTerminated(t))
      .with({ kind: 'watch' }, (c) => this.onWatch(c))
      .exhaustive();
  }

  private onTerminated(signal: Terminated): void {
    this.log.info(`watched actor stopped: ${signal.actor.path}`);
  }

  private onWatch(command: WatchCommand): void {
    this.context.watch(command.ref);
  }
}

const system = ActorSystem.create('demo');

const observed = system.spawnAnonymous(SomeActor);
const watcher  = system.spawnAnonymous(Watcher);

watcher.tell({ kind: 'watch', ref: observed });
observed.stop();   // → watcher logs "watched actor stopped: ..."
```

Three things going on:

1. **`context.watch(ref)`** registers the watcher's interest.  No
   message goes to the watched actor — it doesn't know it's being
   observed.
2. When the watched actor **terminates** (for any reason), the
   framework delivers a `Terminated` message to every watcher.
3. The watcher's `onReceive` handles `Terminated` like any other
   message.  Because it arrives via the mailbox, ordering with
   user messages is well-defined: any `tell`s sent *before* the
   watched actor stopped are processed in order, then the
   `Terminated` follows.

"Like any other message" covers ordering, and only ordering.  A death
is announced **once** — the framework has no way to send it again — so
the notification is exempt from everything that discards user messages
on the way in: a [mailbox bound](/fundamentals/mailboxes/) of any
policy, and a [`throttle({ onExcess: 'drop' })`](/fundamentals/throttling/).  Bounding a watcher
costs it backlog, never the deaths it is watching for.  And if the
notification genuinely cannot be delivered — the watcher itself has
already stopped, or its mailbox refuses the message — it becomes a
[dead letter](/operations/dead-letters/), so the loss is visible
rather than silent.

## The `Terminated` message

```ts
class Terminated {
  constructor(
    public readonly actor: ActorRef,
    public readonly existenceConfirmed: boolean = true,
    public readonly addressTerminated: boolean = false,
  ) {}
}
```

Three fields:

| Field | Meaning |
| --- | --- |
| `actor` | The ref that stopped.  Same instance you passed to `watch`. |
| `existenceConfirmed` | Whether a node saw this actor exist before its termination.  `true` for every local death and for a remote actor that stopped; `false` when you watched a path on another node that resolved to nothing there — the far node answers at once, and the flag is how it says "never saw it". |
| `addressTerminated` | `true` when the notification was synthesised because the actor's whole **node** left the cluster — downed or removed — rather than because the actor itself stopped.  Always `false` for a local death. |

Watching an actor that's already stopped delivers `Terminated`
**immediately** — your watcher always gets a notification, no
matter when you registered.  Locally that notification carries
`existenceConfirmed = true` like every other: the actor did exist,
it is just gone.  The flag only turns `false` across the cluster,
for a path the far node could not resolve, so branch on it there
and nowhere else.

The `addressTerminated` flag belongs to the cluster case, where an
entire node — not just one actor — goes away.  When a node you hold
a watched ref into is downed or removed from the membership, every
watcher of an actor on that node gets a `Terminated` with the flag
set, synthesised from membership rather than sent by the actor.
See [Cluster](/cluster/overview/) for the membership story and
[Watching across the cluster](#watching-across-the-cluster) below.

## Only the runtime can announce a death

`Terminated` is an ordinary exported class, so nothing stops you
constructing one — but constructing one is not the same as causing one.
A `Terminated` the runtime did not emit is **refused**: the receiving
cell consumes it, sends it to
[dead letters](/operations/dead-letters/) with the sender attached, and
leaves the watch registration exactly as it was.

The reason is that the notification carries a ref and nothing else, so
"that actor is dead" is a claim rather than a proof — and honouring an
unfounded one costs the watcher twice.  It acts on a death that has not
happened, *and* the registration goes with it, so the genuine
notification arrives later against a watch that is no longer there and
is dropped as unwatched.  One fabricated message would blind a watcher
to that subject for the rest of its life.

Two consequences worth knowing:

- **Do not forward a `Terminated` to another watcher.**  It will not
  count for them — and it never bought anything: an actor that watches
  the subject already gets its own notification, and one that does not
  was never meant to hear about it.  Send a domain message of your own
  instead.
- **A hand-built `Terminated` does not exercise death watch in a test.**
  Stop the actor for real; the notification that follows is the one your
  handler will see in production.

`watchWith` substitutions are covered by the same rule.  The substituted
message lives in the watcher's own cell and is released by the same
gate, so a fabricated signal cannot quietly consume it and downgrade the
next real death to a `Terminated` the watcher has no arm for.

## Custom termination messages — `watchWith`

`watch` always delivers the same thing.  That is enough when one
death matters, but a watcher that observes several *kinds* of actor
— a pool of workers, a database connection, a cluster peer — gets one
message type for all of them and has to work out from
`Terminated.actor` which relationship just ended.  The signal also has
to live in the watcher's message union, where it says nothing about
what the actor is for.

`watchWith` moves that decision to registration time.  You say what a
death *means* when you start watching, and the watcher receives a
message of its own protocol:

```ts
import { match } from 'ts-pattern';
import { Actor, type ActorRef } from 'actor-ts';

type StartCommand = { kind: 'start' };
type WorkerLostMessage = { kind: 'workerLost'; name: string };
type DatabaseLostMessage = { kind: 'databaseLost' };
type PoolMessage = StartCommand | WorkerLostMessage | DatabaseLostMessage;

class Pool extends Actor<PoolMessage> {
  constructor(private readonly database: ActorRef) {
    super();
  }

  override preStart(): void {
    this.context.watchWith(this.database, { kind: 'databaseLost' });
  }

  override onReceive(message: PoolMessage): void {
    match(message)
      .with({ kind: 'start' }, () => this.onStart())
      .with({ kind: 'workerLost' }, (m) => this.onWorkerLost(m))
      .with({ kind: 'databaseLost' }, () => this.onDatabaseLost())
      .exhaustive();
  }

  private onStart(): void {
    for (let i = 0; i < 4; i++) this.hire(`worker-${i}`);
  }

  private onWorkerLost(message: WorkerLostMessage): void {
    this.log.warn(`${message.name} died — respawning`);
    this.hire(message.name);
  }

  private onDatabaseLost(): void {
    this.log.warn('database gone — winding down');
    this.context.stopSelf();
  }

  private hire(name: string): void {
    const worker = this.context.spawn(Worker, name);
    this.context.watchWith(worker, { kind: 'workerLost', name });
  }
}
```

`Terminated` does not appear in `PoolMessage` at all.  The union stays
a description of what this actor does, `match(...).exhaustive()` still
covers it, and the two deaths that mean completely different things
are handled in two different places.

### The rules

| Call | Effect |
| --- | --- |
| `watchWith(ref, message)` | On `ref`'s termination, `message` is delivered to this actor instead of `Terminated(ref)`. |
| `watchWith(ref, other)` again | Replaces the message — last call wins. |
| `watch(ref)` after a `watchWith(ref, …)` | Drops the custom message; the death delivers `Terminated(ref)` again. |
| `unwatch(ref)` | Removes the registration, whichever of the two made it. |

Everything else is unchanged: the message arrives through the mailbox
with the same ordering guarantees, watching an already-stopped ref
delivers immediately, and a watcher that stops has its registrations
cleaned up for it.  Your domain message inherits the exemption the
`Terminated` it replaces has — a bound or a throttle cannot discard it
either, because it is still the announcement of a death that happens
once.

### One registration, one death

The registration is consumed by the death it describes.  A name that
is re-spawned is a *different* actor as far as death watch is
concerned — the bookkeeping is keyed by incarnation, not by path — so
a replacement child needs its own `watchWith`, which is why `hire()`
above registers every time it spawns.  That is deliberate: it is what
stops a pending notification for the previous incarnation from being
delivered against its successor.

### The message never travels

`watchWith` records `message` in the **watcher**.  The watched actor
is not involved — it still does not know it is being observed, and the
message is neither sent to it nor routed through it.  So there is no
serializer to register and nothing new on the wire: the substitution
happens in the watcher's own cell, at the moment the death is
dispatched to its handler.

### With typed Behaviors

`TypedActorContext` has the same method:

```ts
const pool = Behaviors.setup<PoolMessage>((context) => {
  const worker = context.spawn(workerBehavior, 'worker-0');
  context.watchWith(worker, { kind: 'workerLost', name: 'worker-0' });
  return Behaviors.receiveMessage<PoolMessage>((message) => {
    // ... 'workerLost' arrives here, like any other message
    return Behaviors.same;
  });
});
```

`watchWith` deliberately bypasses `onSignal`.  A plain `watch`
delivers a `{ kind: 'terminated' }` **Signal**, which
`Behaviors.receiveWithSignal` routes to its signal handler; a
`watchWith` message is a value of `T` and goes to the ordinary receive
handler even when a signal handler is registered.  Registering one
does not quietly reroute the message you asked for.

"When a signal handler is registered" is per behavior, not per actor.
A signal handler belongs to the `receiveWithSignal` that declared it,
so a state that is a plain `Behaviors.receive` has none — and a plain
`watch`'s death arrives there as a `Terminated` **message**, exactly as
it does for an actor that never registered a handler at all.  If a
later state still wants the death as a signal, it declares its own
`onSignal`; if it wants it as a message, `watchWith` says so
explicitly and holds in every state.

## `unwatch`

```ts
context.unwatch(ref);
```

Stop receiving termination notifications for this ref — whether the
registration came from `watch` or from `watchWith`.  Idempotent —
calling `unwatch` on a ref you weren't watching is a no-op.

When an actor stops, its watch registrations are cleaned up
automatically; you don't have to `unwatch` everything before
stopping.  Use `unwatch` only when an actor needs to *change* its
interest mid-flight ("I no longer care about this child").

## Common patterns

### Auto-stop on dependency loss

A worker that has no purpose without a particular dependency
should stop itself when that dependency does:

```ts
class Worker extends Actor<Message | Terminated> {
  constructor(private readonly db: ActorRef) {
    super();
  }
  override preStart(): void {
    this.context.watch(this.db);
  }
  override onReceive(message: Message | Terminated): void {
    if (message instanceof Terminated && message.actor === this.db) {
      this.log.warn('DB stopped — winding down');
      this.context.stopSelf();
      return;
    }
    // ... handle Message
  }
}
```

The supervision tree handles failures *inside* the actor; death
watch handles "the actor I depend on disappeared for some other
reason" (parent stop, manual stop from outside, cluster eviction).

### Cleanup coordination

A manager actor that spawns N children and wants to react when all
of them have stopped:

```ts
class Manager extends Actor<Command | Terminated> {
  private alive = new Set<string>();

  override preStart(): void {
    for (let i = 0; i < 4; i++) {
      const child = this.context.spawn(Worker, `worker-${i}`);
      this.alive.add(child.path.name);
      this.context.watch(child);
    }
  }

  override onReceive(message: Command | Terminated): void {
    if (message instanceof Terminated) {
      this.alive.delete(message.actor.path.name);
      if (this.alive.size === 0) {
        this.log.info('all workers stopped — manager exiting');
        this.context.stopSelf();
      }
    }
    // ... handle Command
  }
}
```

This is a common shape for graceful shutdown: a coordinator
watches the actors it's responsible for, and only exits once
every one of them is gone.  The
[Coordinated shutdown](/fundamentals/coordinated-shutdown/)
DSL formalizes this pattern at the system level.

### Watching across the cluster

```ts
const remoteEntity = await this.system.actorSelection(
  'actor-ts://my-app@10.0.0.5:2552/system/cluster/sharding/region-Entity/entity-12345',
).resolveOne(1_000);

this.context.watch(remoteEntity);
```

Watch works the same way across nodes.  Your node sends a `watch`
frame to the node the ref names; that node plants a stand-in in
the watched actor's watcher set, and when the actor stops it
answers with a `watch-terminated` frame that arrives here as the
same branded `Terminated` a local death produces — through the
ordinary `watch` / `watchWith` / `unwatch` calls, with nothing to
configure.  Two things differ from the local case:

- A path the far node **cannot resolve** is answered at once, with
  `existenceConfirmed = false`.  The same answer covers a path the
  far node refuses to hand out by name — `/system/…` always, and
  under `untrusted-mode` any `/user` path off the allow-list — so a
  watcher never hangs on a refusal, and a peer cannot use `watch`
  to probe which actors exist.  See
  [Cluster security](/operations/security/cluster-security/).
- A node that is **downed or removed** sends nothing; your node
  synthesises the `Terminated` from membership, with
  `addressTerminated = true`.  A node that is merely *unreachable*
  does not — it may come back, and the downing provider decides.

A watching actor that stops withdraws its remote watches, and a
node that leaves withdraws every stand-in it planted elsewhere.
See [Refs across nodes](/cluster/refs-across-nodes/) for the rest
of the wire story.

## What it does NOT replace

import { Aside } from '@astrojs/starlight/components';

<Aside type="caution" title="Death watch is not a try/catch">
  ```ts
  this.context.watch(child);
  child.tell({ kind: 'do-it' });
  // If `do-it` throws, the child *restarts* — no Terminated.
  ```
  `Terminated` only fires on **stop**, not on every failure.  An
  actor that crashes and restarts (the default supervisor strategy)
  is still alive — its mailbox keeps draining, no Terminated
  message.  If you want to know about failures, use a custom
  supervisor strategy on the parent; death watch is strictly for
  stop events.
</Aside>

<Aside type="caution" title="`Terminated` for an actor you never watched">
  ```ts
  override onReceive(message) {
    if (message instanceof Terminated) { ... }   // ← will this ever fire?
  }
  ```
  If you never call `context.watch`, `Terminated` doesn't arrive.
  Watching is opt-in, per-watcher.  An actor's own termination
  signal to its supervisor is separate — that flows via the
  internal "child stopped" path, not as a `Terminated` message.
</Aside>

<Aside type="caution" title="Watching `self`">
  `context.watch(this.self)` is legal but pointless.  By the time
  the `Terminated` would fire, the actor has already stopped — its
  mailbox is closed.  The notification routes to dead letters.
  Use `postStop` for cleanup that needs to run when *this* actor
  stops; death watch is for observing *other* actors.
</Aside>

## Ignoring a `Terminated`

A handler that takes the notification and does nothing with it is a
legitimate end state: the actor keeps running, nothing is logged,
nothing fails.  The framework could not react even if it wanted to —
`onReceive` returns `void`, so the cell that dispatched the death has
no way to tell an ignored `Terminated` from a handled one.

"Ignored" means *the handler ran and did nothing*, though.  A death the
handler never reaches is a different story, and the APIs answer it
differently:

| Watcher | A `Terminated` the handler does not act on |
| --- | --- |
| `Actor.onReceive` with no branch for it | Nothing happens; the actor keeps running. |
| `match(message)` … `.exhaustive()` with no arm for it | ts-pattern throws.  That is an ordinary failure, so it goes through supervision — under the default strategy the watcher **restarts**. |
| `Behaviors.receiveWithSignal` whose signal handler answers `Behaviors.unhandled` | Nothing happens. |
| A typed behavior with no signal handler, whose receive answers `Behaviors.unhandled` | The `Terminated` goes to dead letters. |

The second row is the one that catches people out: `.exhaustive()` is
the idiom this page uses everywhere, and it turns "this death does not
concern me" into a restart loop unless an arm covers it.  Add the arm,
or don't `watch` in the first place.

### `DeathPactError` is yours to throw

Some watchers genuinely cannot carry on — a session whose connection
died, a saga step whose worker vanished.  `DeathPactError` is exported
for exactly that, and **the runtime never raises it**: there is no
automatic death pact in actor-ts.  You throw it yourself, from your own
handler, and it travels through supervision like any other failure:

```ts
import { match, P } from 'ts-pattern';
import {
  Actor,
  ActorOptions,
  DeathPactError,
  decideBy,
  Directive,
  OneForOneStrategy,
  Terminated,
  type ActorRef,
} from 'actor-ts';

type QueryCommand = { kind: 'query'; sql: string };
type SessionMessage = QueryCommand | Terminated;

class Session extends Actor<SessionMessage> {
  constructor(private readonly connection: ActorRef) {
    super();
  }

  override preStart(): void {
    this.context.watch(this.connection);
  }

  override onReceive(message: SessionMessage): void {
    match(message)
      .with(P.instanceOf(Terminated), (t) => this.onTerminated(t))
      .with({ kind: 'query' }, (c) => this.onQuery(c))
      .exhaustive();
  }

  /** No connection, no session — break the pact and let the parent decide. */
  private onTerminated(signal: Terminated): void {
    throw new DeathPactError(signal.actor.path.toString());
  }

  private onQuery(command: QueryCommand): void {
    // ... run it against the connection
  }
}

// A restart is the wrong answer for a broken pact: the dependency is gone,
// so a fresh instance fails the same way.  Say so, or the default strategy
// will restart it ten times a minute.
const pactStrategy = new OneForOneStrategy(
  decideBy([{ match: DeathPactError, then: Directive.Stop }]),
);
const sessionOptions = ActorOptions.create().withSupervisorStrategy(pactStrategy);

const session = system.spawn(() => new Session(connection), 'session-42', sessionOptions);
```

`actorPath` carries the path of the actor whose death broke the pact,
so a watcher that depends on several actors can tell from the error
alone which dependency it lost.

Leaving the throw to you is the point rather than a gap.  A death pact
is a policy about *this* watcher — two actors watching the same worker
may well want different answers — and the framework cannot pick one for
them: `onReceive` gives it no signal to distinguish a handler that
ignored the death from one that dealt with it quietly.

## Watch + supervision — when to use which

The two mechanisms cover different cases:

- **Use supervision** when the actor handling failures is the
  *parent* of the actor that fails.  Restart/Resume/Stop/Escalate
  per error class is what supervision is for.
- **Use death watch** when the observer is **not** the parent —
  a sibling that needs to know when a dependency goes away, a
  cross-tree manager watching workers it spawned, an HTTP handler
  noticing that the backend actor died.

Many actors use both: supervise their *own* children, and watch
the actors they *depend on* (which are someone else's children).

## Where to next

- **[Supervision](/fundamentals/supervision/)** — the
  parent-handles-child-failure mechanism.  Death watch is the
  observer-handles-actor-stop mechanism — distinct, often used
  together.
- **[Poison pill and Kill](/fundamentals/poison-pill-and-kill/)** —
  two ways to terminate an actor.  Poison pill stops it (a
  `Terminated` follows); `Kill` raises an error through supervision,
  so under the default restart strategy the actor restarts and no
  `Terminated` fires unless the strategy resolves to Stop.
- **[Coordinated shutdown](/fundamentals/coordinated-shutdown/)** —
  uses watch internally to wait for whole subsystems to drain.
- **[Refs across nodes](/cluster/refs-across-nodes/)** —
  how `Terminated` propagates when the watched actor lives on a
  different node.

The [`ActorContext.watch`](/api/interfaces/actorcontext/)
and [`Terminated`](/api/classes/terminated/) API references
cover the full signatures.
