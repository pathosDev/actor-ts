...
When you call `system.spawn(props, name)`, the actor is created under
...
3. Stop `/system` — framework internals unwind.  This starts only once
   `/user` is fully drained, so a `postStop` in step 2 can still talk to
