...
(`${?ENV_NAME}`) works in HOCON the same way as Akka — values pulled
from the environment fall back to the default when unset.  See
[Configuration](/actor-ts/reference/configuration/) for every key
the framework reads.
...
parent's [supervisor strategy](/actor-ts/fundamentals/supervision/).
Top-level actors (from `system.actorOf`) escalate to the system's
...
When you call `system.actorOf(props)`, the actor is created under
...
<Aside type="caution" title="Spawning before `await system.extension(...).start(...)` resolves">
  ...
  warm-up before they're usable.  If you `system.actorOf(...)` an
...
- **[Actor](/actor-ts/fundamentals/actor/)** — the class you'll
  spawn into the system.
- **[Coordinated shutdown](/actor-ts/fundamentals/coordinated-shutdown/)** —
  graceful-shutdown DSL beyond a plain `terminate`.
- **[Cluster overview](/actor-ts/cluster/overview/)** — when you go
...
The [`ActorSystem` class API reference](/actor-ts/api/classes/actorsystem/)
