...
published on the event stream as a `DeadLetter` naming the recipient it
failed to reach, and that is **all** that happens by default: nothing
logs them and nothing keeps them, so with no subscriber the message is
gone.  Subscribe to react programmatically, or turn on the bounded —
optionally durable — queue that keeps and replays them; see
