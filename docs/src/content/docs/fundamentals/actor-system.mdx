...
- a mailbox parked by `context.throttle(...)`, and one suspended while
  a supervisor decides — neither drains at a rate a shutdown can wait
  for, so both count as quiet;
