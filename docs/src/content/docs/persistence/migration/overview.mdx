...
| Tool | When |
| --- | --- |
| **[Envelope format](/actor-ts/persistence/migration/envelope-format/)** | Every persisted event carries a version tag.  Enables migrating. |
| **[Schema registry](/actor-ts/persistence/migration/schema-registry/)** | Optional — declares known schemas + their versions. |
| **[DefaultAdapter](/actor-ts/persistence/migration/default-adapter/)** | Auto-fill defaults for fields added in a newer version. |
| **[MigratingAdapter](/actor-ts/persistence/migration/migrating-adapter/)** | Chain transformations from v1 → v2 → v3. |
| **[WrapLegacy](/actor-ts/persistence/migration/wrap-legacy/)** | Upgrade un-envelope'd legacy events to versioned envelopes. |

Plus a more focused page: [Recipes](/actor-ts/persistence/migration/recipes/)
...
See [Envelope format](/actor-ts/persistence/migration/envelope-format/)
for the details.
...
```
Is the change ADDITIVE (new fields only, with sensible defaults)?
├── Yes      → DefaultAdapter (zero-effort)
├── No, but transformable from old → new
│   ├── Single-step  → MigratingAdapter
│   └── Multi-step   → MigratingAdapter (chain)
└── Restructured (renames, splits, joins) → Custom EventAdapter
```
...
See [Schema registry](/actor-ts/persistence/migration/schema-registry/).
...
and [Recipes](/actor-ts/persistence/migration/recipes/).
...
- **[Schema registry](/actor-ts/persistence/migration/schema-registry/)** —
