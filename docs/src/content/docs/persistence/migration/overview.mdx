...
| **[defaultsAdapter](/persistence/migration/default-adapter/)** | Auto-fill defaults for fields added in a newer version. |
| **[migratingAdapter](/persistence/migration/migrating-adapter/)** | Chain transformations from v1 → v2 → v3. |
| **[Wrap legacy](/persistence/migration/wrap-legacy/)** | Bulk-wrap un-envelope'd legacy events into versioned envelopes. |
...
flowchart TB
  ...
  DEF["defaultsAdapter<br/><i>zero-effort</i>"]
...
This is a one-shot rewrite of the stored data; afterwards your
normal adapter replays the now-enveloped v1 events.  See
