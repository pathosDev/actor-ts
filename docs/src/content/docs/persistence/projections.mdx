...
### The same knobs in HOCON

Failure handling is usually an *operational* decision — the same code
wants `retry-and-fail` in production and `skip` in a dev loop — so
the four fields, and the poll cadence, also have process-wide
defaults under `actor-ts.projection`:
...
Precedence is the project's usual three layers, applied **per
field**: explicit `ProjectionOptions` > HOCON > built-in default.  A
field this projection does not set falls through to the block above,
and one the block does not carry falls through to the built-in value
— so the configuration reaches every projection that has not made up
its own mind about that field, and none that has.  Passing an empty
`liveOptions` is *not* making up your mind: `poll-interval` still
applies.

Set `max-retry-backoff` whenever you set `retry-backoff`.  The two
are checked against each other on the *merged* settings, so raising
only the first past the 60 s ceiling makes every projection in the
process throw `OptionsError` when it starts — from a config file
...
In a cluster, the offset store needs the same property the journal
does: **one database, not one per node.**  A
`DurableStateOffsetStore` over node-local storage gives every node
its own cursor, and a projection that moves between nodes then
re-processes events one node already handled — or skips the ones
the other node's cursor is past.  See [storage locality &
...
`actor-ts.projection.poll-interval` sets the same cadence for every
projection in the process; `withLiveOptions` overrides it for one.
