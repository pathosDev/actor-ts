...
// Production (durable — reuses any DurableStateStore backend):
...
- `DurableStateOffsetStore` — persists offsets via any
  `DurableStateStore` (Postgres, MariaDB, object-storage), so you
  reuse a durable backend you already run.
