...
// MariaDB, libSQL, SQL Server, MongoDB, object-storage), so a restart resumes mid-stream:
...
- `DurableStateOffsetStore` — wraps any `DurableStateStore` (Postgres,
  MariaDB, libSQL, SQL Server, MongoDB, object-storage) so offsets survive restarts.
