...
See [PersistentActor](/actor-ts/persistence/persistent-actor/) for
the full surface.
...
See [DurableStateActor](/actor-ts/persistence/durable-state/) for
the full API.
...
- **You want projections** — read-side views over the event
  stream, see [Projections](/actor-ts/persistence/projections/).
...
See [Projections](/actor-ts/persistence/projections/) for the
full pattern.
...
[`persistence/journals/*`](/actor-ts/persistence/journals/in-memory/)
and [`persistence/snapshot-stores/*`](/actor-ts/persistence/snapshot-stores/in-memory/).
...
- **[Journals — In-memory](/actor-ts/persistence/journals/in-memory/)** —
...
The [`PersistentActor`](/actor-ts/api/classes/persistentactor/) and
[`DurableStateActor`](/actor-ts/api/classes/durablestateactor/) API
