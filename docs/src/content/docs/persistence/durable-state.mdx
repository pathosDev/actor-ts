...
at them ([#960](https://github.com/mknorr/actor-ts/issues/960)):
on in-memory, SQLite, Postgres, Mongo and DynamoDB, an override
here is dropped silently — no encryption, no tamper detection, and
no error telling you so.  Check the override against the store you
actually run before treating it as a control that holds.
