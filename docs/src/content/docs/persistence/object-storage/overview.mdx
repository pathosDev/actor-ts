...
Two limits are worth knowing, because the alternative is assuming
more than the check delivers.  It reads the filesystem and then
acts on it, so it refuses a link that is already in place but
cannot win a race against one planted in between — portable
`O_NOFOLLOW` is not reachable through `node:fs`.  And it confines
the backend's *own* operations; it is not a sandbox around a
directory other local processes may write to.  If untrusted users
can create entries inside the storage root, that is already the
larger problem.
