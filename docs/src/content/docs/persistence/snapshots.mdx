...
| `PostgresSnapshotStore` | PostgreSQL-backed production. |
| `MariaDbSnapshotStore` | MariaDB / MySQL-backed production. |
| `LibSqlSnapshotStore` | libSQL / Turso — remote SQLite over HTTP. |
| `MsSqlSnapshotStore` | Microsoft SQL Server-backed production. |
| `MongoSnapshotStore` | MongoDB-backed production. |
| `DynamoDbSnapshotStore` | DynamoDB-backed production. |
| `D1SnapshotStore` | Cloudflare D1 — SQLite at the edge. |
| `CassandraSnapshotStore` | Distributed — Cassandra / ScyllaDB. |
