---
title: Supply chain
description: What the release pipeline guarantees about a published actor-ts version — npm provenance, a CycloneDX SBOM on every release, CodeQL analysis and an advisory gate over the real lockfile — and how to verify each of them yourself.
---
...
| **CycloneDX SBOM** | Attached to the GitHub Release as `actor-ts-sbom.cyclonedx.json` | The exact dependency closure that was installed when the release was built. |
...
The SBOM is a release asset, so it does not need an install:
...
Four checks stand between a change and a release. All of them run
on pull requests; the two that can go stale without anybody
