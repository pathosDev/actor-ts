---
title: Supply chain
description: What the release pipeline guarantees about a published actor-ts version — npm provenance, a CycloneDX SBOM from the next release onward, CodeQL analysis and an advisory gate over the real lockfile — and how to verify each of them yourself.
---
...
<Aside type="note" title="The SBOM starts with the next release">
The two jobs that build and attach it landed in `publish.yml` *after*
v0.16.0 was published, so no release you can download today carries the
asset — `gh release view v0.16.0 --json assets` answers with an empty
list. The first release cut by the current pipeline is the first one to
carry it. The other two rows hold for every release already published.
</Aside>
