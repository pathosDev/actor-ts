...
| Gossip frame captured off the wire and played back later | Per-sender frame sequence — a frame must out-number the last one accepted |
...
### A recorded frame cannot be played back while its sender is a member
...
Two things this does **not** close. First, a peer that has earned
...
Second, the mark lives only as long as the sender does. Deleting a
member drops its mark, and a missing mark admits everything, so
once the **sender** has itself been evicted (an ordinary partition
takes out sender and subject together) the recorded conversation
can be replayed in capture order: the sender's own self-record is
always admitted, which revives it *and* re-seeds the mark from the
old frame, and the next recorded frame then speaks for the
subject. An incarnation identity does not close this one either —
a node downed while still running answers to the same incarnation
the recording carries.

Both residuals are tracked separately.
