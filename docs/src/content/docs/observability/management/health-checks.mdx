...
| `cluster-transport` | readiness | `Cluster.join` | the node has **no** transport connection to **any** peer it still expects |
...
membership view alone, because nothing else monitors the local
transport and no peer is allowed to downgrade this node's own
record.  A single-node cluster expects nobody and always passes.

`cluster-membership` and `cluster-transport` are removed again by
`cluster.leave()`.
