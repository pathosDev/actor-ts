...
Everything above except the two middleware fields also has a
config form, so the same wiring can come from `application.conf`
and differ per environment without a rebuild:
...
The usual three layers apply — an option set in code wins, the
file fills what the code left unset, the built-in defaults are the
floor — so `managementRoutes(system, cluster)` with no third
argument at all is the whole wiring when the file carries it.

**`auth` and `ipAllowlist` are the exception and have no key.**
They are `Middleware` functions, which HOCON cannot express, so
the block decides *which* endpoints exist and *where* the probes
answer — never *who* may reach them.  That is what makes
config-switchable admin endpoints safe: a file cannot loosen the
access control in front of them.  Full key table:
[Configuration](/reference/configuration/#actor-tsmanagement).
...
| `GET /cluster/shards?type=<name>` | When `cluster` is set, for a type this node participates in | Shard placement for a sharded type. |
