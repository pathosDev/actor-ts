...
The `checks` array holds every readiness check on the system's
...
### Moving the two probe paths

`/health` and `/ready` are defaults, not fixed routes.  Each is a
single path segment, settable in code or from a config file:
...
A value containing `/` is refused with an `OptionsError` when the
routes are built, because `path()` strips only *surrounding*
slashes — `"k8s/ready"` would become one segment holding a slash,
match no URL, and take the endpoint out of existence silently.
Whatever the segments are, keep the K8s probe paths in step with
them; nothing cross-checks a manifest.
...
- `version` counts coordinator *broadcasts*, not shard assignments —
  one bump per publish, and a publish coalesces a burst of moves.
  Compare it across two readouts to tell "the map moved" from "the map
  is the same".
- `takenAt` and `leader` are stamped by the answering node when the
  map arrived, so two nodes can report the same `version` with
  slightly different timestamps.
- `regions[].key` is `<address>|<region path>`, the same key
  `shardHome[].regionKey` uses.  A region that hosts nothing is still
  listed, with an empty `shards` — "registered and given no shards" is
  the interesting half of a rebalance question.
- A **200 with an empty `shardHome`** is normal, not an error:
  `regions` fills as soon as regions register, while a shard only gets
  a home once an entity in it is addressed.
...
[prom-client bridge](/observability/metrics/prom-client-adapter/) is
...
A `200` with an empty body would be worse than the error: an empty
