---
title: Pool vs group
description: Two ways routers acquire routees — **pool** routers spawn their routees from an actor class or factory (the local Router); **group** routers find existing routees by path (ClusterRouter). The trade-off in lifecycle ownership and the kinds of routees each fits.
---
...
| --- | --- | --- |
| **Pool** | The router — it spawns the routees itself. | Static at spawn time; size doesn't change without a router restart. |
| **Group** | Someone else — the routees already exist. | Dynamic (group routers re-discover when needed). |

The local `Router` is **always a pool** — including
[`Router.scatterGatherFirstCompleted`](/routing/scatter-gather/),
which spawns and owns its routees like every other local factory.  The
cluster `ClusterRouter` is **always a group** (routees are derived from
cluster up-members at a well-known path).  Knowing which you have
shapes how you think about lifecycle, supervision, and resize.
...
- **Supervision is straightforward.**  The router supervises its
  routees with whatever strategy the routee options declare (or
  the default).
...
Right shape for **homogeneous worker pools** — N stateless workers,
all the same, keeping one stream of I/O-bound work flowing.  That is
concurrency on the router's thread, not CPU parallelism; for the pool
that spans threads see
[One address, one actor per thread](/routing/overview/#one-address-one-actor-per-thread).
...
- **Lifecycle is independent.**  Stop the router → routees keep
  running.  Stop a routee → the router does not notice: routees are
  derived from *membership*, not probed for liveness, so it keeps
  sending that node its share until the node itself leaves — and the
  node drops what arrives for a path nobody has, with a warning (the
  cluster's own `no envelope handler registered`, or, on a node that
  has started `ClusterSharding`, the sharding extension's `[sharding]
  no region/coordinator registered for …` — it takes every unresolved
  envelope on that node).  The rest of the pool is unaffected.
...
`ClusterRouter` that targets them by path.  On the worker mesh that
composition is a documented recipe — the actor module's `setup` spawns
the routee on every worker, and a `ClusterRouter` on the main thread
finds them by path and role; see
[One address, one actor per thread](/routing/overview/#one-address-one-actor-per-thread).
For a cluster of processes, see
...
| One routee | Crashed → restarted in place by the supervisor (default strategy). Stopped → pruned on its `Terminated`: the pool shrinks by one and nothing is routed into the gap (#449). | Router keeps sending to that node until its membership changes — it does not probe routee liveness — and the node drops that share with a warning (the cluster's, or the sharding extension's on a node that has started `ClusterSharding`); siblings unaffected. |
...
- **[Scatter/gather](/routing/scatter-gather/)** — the
  reply-shaped pool router, pool-only for the same reasons.
