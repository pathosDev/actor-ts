...
| One routee | Crashed → restarted in place by the supervisor (default strategy). Stopped → pruned on its `Terminated`: the pool shrinks by one and nothing is routed into the gap (#449). | Router keeps sending to that node until its membership changes — it does not probe routee liveness — and the node drops that share with a warning; siblings unaffected. |
