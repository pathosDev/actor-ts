...
- **An empty pool drops messages with a warning.**  If no node
  ...
  letters.  See the
  [Cluster router](/actor-ts/cluster/cluster-router/) docs for
  the exact semantics.
...
The pool/group distinction is sometimes muddied by Akka's
`ClusterPool`, which spawns its own routees but *across* cluster
nodes (one routee per node).  actor-ts doesn't have this — the
cluster router is always group-shaped.  If you want a fixed pool
...
- **[Router](/actor-ts/routing/router/)** — the local pool-router
  API.
...
- **[Sharded daemon process](/actor-ts/cluster/sharding/sharded-daemon-process/)** —
  ...
  "cluster pool."
- **[Strategies](/actor-ts/routing/strategies/)** — the routing
