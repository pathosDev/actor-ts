...
| One routee | Supervisor restarts it (default); if stopped, its dead ref stays in the pool and round-robin keeps routing it ~1-in-N messages → dead letters. | Router stops sending to it (after cluster notices), siblings unaffected. |
