...
- **Two never had one.**  `peers list is empty after shutdown` is synchronous
  end to end: every step is awaited, there is no poll and no delay, so the
  remaining suspect is the module-level registry `InMemoryTransport` shares
  between tests.  `ParallelMultiNodeSpec.test.ts` contains **no** fixed-delay
