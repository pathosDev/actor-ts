...
5. Revisit `test.yml`'s pragmatic `COVERAGE_LINE_FLOOR: '80'`, lowered for
   the same reason — and the per-module floors in `scripts/coverage-gate.mjs`
   with it.  `src/cluster/` measures below its true line coverage for as long
   as `LeaseMajority` cannot run, so its floor is set against the quarantined
   population and not against the real one.
