...
<Aside type="caution" title="`expectMsg` still has a real-time timeout">
  ```ts
  await probe.expectMsg('tick');   // 3s real timeout by default
  ```
  The probe's `expectMsg` waits on real time for the message to
