...
incoming duplicates per `(producerId, producer incarnation, seq)` — but only
within its in-memory lifetime, and only within one incarnation of the producer.
If you need dedup that survives either side restarting, persist your own
processed-seq alongside the business state in the handler:
