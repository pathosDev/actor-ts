...
- Dedups duplicates (same `producerId` + producer incarnation + `seq`),
  ...
  producers, and entries idle for `producerIdleTtlMs` are swept.
...
bound wire input, and a limit the sender could raise is not a limit.  The two
...
handler resolves and the Ack lands.  What *is* bounded is the dedup
bookkeeping behind it: see [Why the dedup state has a
budget](#why-the-dedup-state-has-a-budget).  Losing an entry never drops a
message; it costs a duplicate.
