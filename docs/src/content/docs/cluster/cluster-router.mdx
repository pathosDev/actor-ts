...
The router derives its routee set from
`Cluster.placementCandidates()` — the up-members this node is
willing to put work on.  Triggers a rebuild on:
...
### Candidates, not simply up-members

`placementCandidates()` **is** `upMembers()` while
`actor-ts.cluster.configuration-compatibility-check.enforce` is
`off`, which is the default — so most deployments can read the two
as one set.

With enforcement on, a peer whose
[checked configuration](/cluster/joining-and-seeds/#settings-that-have-to-agree)
disagrees with this node's drops out of the set and the router stops
sending to it.  The peer is not downed and stays `up` for every
other purpose; only placement changes.

Routee selection is the **only** consumer of that set, deliberately.
The view is per-node — each node compares what its peers gossip
against its own resolved values — so it is asymmetric by
construction: with A and B agreeing and C diverging, C excludes both
of them while they exclude only C.  That costs a routee here, and it
would produce two winners anywhere a role is *elected*, which is why
[cluster singletons](/cluster/singleton/overview/) and the shard
coordinator keep reading up-members.

A divergence is not a membership event, so a router that is already
running applies it at its next rebuild — the next `MemberUp` or
`MemberRemoved`.  In the ordinary case that is not a wait: the
settings the framework itself publishes are resolved before a node's
first frame, so a diverging peer is already outside the candidate
set by the time joining it fires `MemberUp`.
...
Only candidates carrying the `compute` role are routees.
...
`role` field narrows the candidate set; without it, **every
candidate is a routee**.
