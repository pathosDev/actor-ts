...
The local [`Router`](/actor-ts/routing/router/) creates its routees
...
See [Pool vs group](/actor-ts/routing/pool-vs-group/) for the
...
[Strategies](/actor-ts/routing/strategies/) for the picking
...
stop.  See [Pool vs group](/actor-ts/routing/pool-vs-group/).
...
<Aside type="caution" title="Consistent-hashing with skewed keys">
  ...
  consider [sharding](/actor-ts/cluster/sharding/overview/) (one
  actor per key, all isolated) or aggregating before routing.
</Aside>

## Where to next

- **[Routing overview](/actor-ts/routing/overview/)** — the
  bigger routing picture.
- **[Pool vs group](/actor-ts/routing/pool-vs-group/)** — the
...
The [`ClusterRouter`](/actor-ts/api/classes/clusterrouter/) API
