...
- **`weakly-up`** *(optional)* — gossip-visible to peers but the
  ...
  see [Weakly-up](/actor-ts/cluster/weakly-up/).
...
[Failure detector](/actor-ts/cluster/failure-detector/) for the
phi math.
...
See [Downing strategies](/actor-ts/cluster/downing-strategies/) for
the full set.  Pick deliberately — the default (no downing
strategy) requires manual intervention during a partition.

## Cross-node messaging

Once two nodes share a cluster, `ref.tell(msg)` to a foreign-node
ref **just works**:
...
| --- | --- |
| **[Sharding](/actor-ts/cluster/sharding/overview/)** | One actor per "entity key," distributed across nodes, with automatic rebalancing on membership changes. |
| **[Singleton](/actor-ts/cluster/singleton/overview/)** | One actor cluster-wide.  Re-spawned elsewhere if the host node leaves. |
| **[DistributedPubSub](/actor-ts/cluster/pubsub/)** | Topic-based fan-out across the cluster. |
| **[DistributedData](/actor-ts/distributed-data/overview/)** | CRDT-based shared state with eventual consistency. |
| **[Cluster router](/actor-ts/cluster/cluster-router/)** | Routes messages across cluster-up-members at a well-known path. |
| **[Receptionist](/actor-ts/discovery/receptionist/)** | Service registry — actors register, others look up by key. |
...
- **[Failure detector](/actor-ts/cluster/failure-detector/)** — the
  phi-accrual mechanism.
- **[Downing strategies](/actor-ts/cluster/downing-strategies/)** —
  resolving split-brain.

The [`Cluster`](/actor-ts/api/classes/cluster/) API reference covers
