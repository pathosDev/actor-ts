...
| Setting | Purpose |
| --- | --- |
| `host` + `port` | The interface this node **binds**.  `0.0.0.0` means "every interface" and is the default. |
| `advertisedHost` | The address peers **dial** — this node's identity.  Unset, it is derived from `host` when that is routable, then from `CLUSTER_HOST` / `POD_IP` / `HOSTNAME`, then loopback. |
