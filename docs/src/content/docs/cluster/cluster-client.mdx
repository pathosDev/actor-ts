...
const client = new ClusterClient(
  ClusterClientOptions.create()
    .withContactPoints(['actor-ts://my-app@10.0.0.5:2552/system/receptionist']),
);
...
sequenceDiagram
  ...
  participant Cluster
  Ext->>Cluster: contact cluster receptionist via well-known path
  Cluster-->>Ext: known service refs
  Ext->>Cluster: send messages to refs
```
...
const client = new ClusterClient(ClusterClientOptions.create().withContactPoints([...]));
