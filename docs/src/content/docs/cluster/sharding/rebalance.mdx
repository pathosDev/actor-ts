...
```
         coordinator                 source region         destination region
              │                            │                       │
              ├── HandOff(shardId) ───────►│                       │
              │                            │                       │
              │           (drain entities + buffer messages)        │
              │                            │                       │
              │ ◄── HandOffComplete ───────┤                       │
              │                            │                       │
              ├── allocate new home ──────────────────────────────►│
              │                                                    │
              │            (region accepts shard, spawns entities  │
              │             as messages arrive)                    │
```
...
sharding.start({
  // ...
  rebalanceIntervalMs: 2_000,   // strategy-driven rebalance every 2s
  handOffTimeoutMs:    10_000,  // give up + force-reallocate after 10s
});
```
...
For persistent entities ([PersistentActor](/actor-ts/persistence/persistent-actor/)):
