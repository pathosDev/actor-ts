...
| Actor | Role |
| --- | --- |
| **Region** | One per node.  Routes messages to the right shard's owner, and decides when local entities passivate. |
| **Shard** | One per shard hosted here, a child of the region.  Owns the entity actors: spawns them, stops them, buffers for one on its way out. |
...
Four of these are also HOCON keys — `number-of-shards`,
`remember-entities`, `passivation-idle` and `max-entities` — as are the
coordinator's `rebalance-interval` and `hand-off-timeout`.  They set the
node-wide baseline for every sharded type and sit *under* whatever
`start(...)` passes explicitly, so a per-type value in the builder still
wins.  See [Configuration](/reference/configuration/#actor-tssharding).
...
There is **no default idle window** — `passivationIdleMs` is `0`
(disabled) unless you set it, in code or via
`actor-ts.sharding.passivation-idle`, and an entity that nobody
passivates stays resident for the lifetime of its node.
