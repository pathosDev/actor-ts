...
```ts
import { Actor, Cluster } from 'actor-ts';
...
const { system, cluster } = await Cluster.bootstrap({ name: 'my-app' });
...
cluster.sharding.start('cart', CartActor, { extractEntityId: (m: CartCmd) => m.entityId });
...
cluster.sharding.start<CartCmd>({
  typeName:        'cart',
