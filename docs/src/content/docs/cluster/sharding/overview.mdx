...
`remember-entities`, `passivation-idle`, plus the coordinator's
`rebalance-interval` and `hand-off-timeout`.  They set the node-wide
baseline for every sharded type and sit *under* whatever `start(...)`
passes explicitly, so a per-type value in the builder still wins.  See
