...
class CartActor extends Actor<CartCommand> {
  ...
  override onReceive(cmd: CartCommand): void {
...
const startShardingOptions = StartShardingOptions.create<CartCommand>().withExtractEntityId((msg) => msg.entityId);
...
class CartActor extends Actor<CartCommand | Passivate> {
