...
id, though: actor names have a restricted alphabet, so the shard folds
every character outside `[A-Za-z0-9_-]` to `_` when it names the child.
`user:42` and `user/42` both land on `entity-user_42`.  The path is a
*label*; `entityId` is the value.
...
and replayed exactly as they are for an entity.  One consequence worth
knowing: asking for a passivated shard's ref with
[`shardRefFor()`](/cluster/sharding/introspection/) brings it back, since
a handle to an actor that is not running would be a handle to nothing.
