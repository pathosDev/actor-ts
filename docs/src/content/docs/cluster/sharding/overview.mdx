...
[ShardRegion](/actor-ts/cluster/sharding/), the
[allocation strategy](/actor-ts/cluster/sharding/allocation-strategy/),
and [rebalance](/actor-ts/cluster/sharding/rebalance/) for the
mechanics.
...
See [Rebalance](/actor-ts/cluster/sharding/rebalance/) for the
full protocol.
...
- **`PersistentActor`** — the entity persists events to a
  ...
  See [PersistentActor](/actor-ts/persistence/persistent-actor/).
- **`DurableStateActor`** — simpler: persist the current state
  snapshot; restore on restart.  See
  [DurableState](/actor-ts/persistence/durable-state/).
...
<Aside type="caution" title="No natural key">
  ...
  [ClusterSingleton](/actor-ts/cluster/singleton/overview/) instead
...
- **[ShardRegion](/actor-ts/cluster/sharding/) ** *(stub)* — the
...
The [`ClusterSharding`](/actor-ts/api/classes/clustersharding/)
