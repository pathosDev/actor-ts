...
| Provider | When |
| --- | --- |
| **[Config](/actor-ts/discovery/seed-providers/config/)** | Static list (the case above). |
| **[DNS](/actor-ts/discovery/seed-providers/dns/)** | Resolves `_actor-ts._tcp.example.com` SRV records. |
| **[Kubernetes API](/actor-ts/discovery/seed-providers/kubernetes-api/)** | Lists pods matching a label selector. |
| **[Aggregate](/actor-ts/discovery/seed-providers/aggregate/)** | Falls through multiple providers (e.g. K8s, then DNS). |
...
[Discovery overview](/actor-ts/discovery/overview/) for the seed
provider model.
...
<Aside type="caution" title="Seeds unreachable, no self in list">
  ...
  Cluster.join(system, { host: 'n4', port: 2552, seeds: ['n1', 'n2', 'n3'] });
  ```
...
<Aside type="caution" title="Two clusters forming with the same name">
  ...
  with a downing strategy ([downing-strategies](/actor-ts/cluster/downing-strategies/))
...
- **[Cluster overview](/actor-ts/cluster/overview/)** — the
  bigger picture.
- **[Weakly-up](/actor-ts/cluster/weakly-up/)** — gradual-join
