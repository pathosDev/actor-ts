...
The retry cadence is `withAcquireRetryIntervalMs(…)`, or
`actor-ts.cluster.singleton.acquire-retry-interval` (default 5 s) for the
whole node.  It is worth setting from the config file even though the lease
itself has no key: until an acquire succeeds nothing hosts the singleton, so
this interval is most of the recovery latency of a partition that heals.
...
- **Buffering at the sender** — the singleton proxy buffers while
  ...
  is capped (`withBufferSize`, or
  `actor-ts.cluster.singleton.buffer-size` — the only channel a node
  that just calls `ref` has; default 1000); past the cap messages
