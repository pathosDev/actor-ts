...
The
[singleton manager](/actor-ts/cluster/singleton/manager/) by
...
```
Cluster gossip says: A is the leader, then network partition...
       │
       │ both A and B see themselves as leader
       ▼
   manager_A.acquire(lease) ──► race ──► manager_B.acquire(lease)
                                    │
                          ┌─────────┴─────────┐
                          ▼                   ▼
                  acquire wins for A    acquire fails for B
                          │                   │
                          ▼                   ▼
                    spawn singleton      stay passive
       │                                       │
       │ partition heals; gossip converges     │
       │ acquire's onLost fires on retired side │
```
...
[Coordination](/actor-ts/coordination/overview/) — InMemoryLease
...
<Aside type="caution" title="Lease in mTLS-only environments">
  ...
  ```
  See [KubernetesLease](/actor-ts/coordination/kubernetes-lease/).
</Aside>
...
- **[Singleton overview](/actor-ts/cluster/singleton/overview/)** —
  the foundation.
- **[Singleton manager](/actor-ts/cluster/singleton/manager/)** —
  the per-node election logic.
- **[Sharding with lease](/actor-ts/cluster/sharding/with-lease/)** —
  the same pattern for sharding.
- **[Coordination](/actor-ts/coordination/overview/)** — the
  lease abstraction.
