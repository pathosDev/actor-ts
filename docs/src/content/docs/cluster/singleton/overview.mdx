...
- **`DurableState`** — simpler: snapshot the current state;
  restore on restart.  See [DurableState](/actor-ts/persistence/durable-state/).
...
a [cluster router](/actor-ts/cluster/cluster-router/) or
[sharding](/actor-ts/cluster/sharding/overview/) instead — both
...
<Aside type="caution" title="Per-key state that scales">
  ...
  [ClusterSharding](/actor-ts/cluster/sharding/overview/) instead —
...
- **[ClusterSingletonManager](/actor-ts/cluster/singleton/manager/)** —
  the per-node manager that elects + spawns the singleton.
- **[Singleton with lease](/actor-ts/cluster/singleton/with-lease/)** —
  split-brain protection via a coordination lease.
- **[Coordination](/actor-ts/coordination/overview/)** — the lease
  abstraction itself.
...
The [`ClusterSingletonManager`](/actor-ts/api/classes/clustersingletonmanager/)
and [`ClusterSingletonProxy`](/actor-ts/api/classes/clustersingletonproxy/)
