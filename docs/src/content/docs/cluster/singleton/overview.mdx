...
// On every node: start the singleton via the ClusterSingleton extension.
// One call spawns this node's manager and builds the proxy; only the
// leader's manager actually constructs the JobScheduler.
const startSingletonOptions = StartSingletonOptions.create<JobCommand>()
  ...
  .withRole('control-plane');   // optional: only these nodes can host
