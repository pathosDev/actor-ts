...
host — a leader change, but equally a member coming up, going
unreachable, or leaving.  That full set matters for a role-restricted
singleton: the first member carrying the role can join without the
leader changing at all, and before v0.17.0 that left the buffer
undrained indefinitely while every later send routed normally.
