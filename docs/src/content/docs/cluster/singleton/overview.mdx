...
For `role` or `lease`, pass options alongside — or use the full builder
form when there is no class to hang a static on:

```ts
const singletonOptions = StartSingletonOptions.create<JobCommand>()
  .withRole('control-plane');   // only these nodes can host
