...
`typeName`, `singletonProps` and the three above — are assembled from
...
| `singletonProps` | Yes | How to construct the singleton.  Only invoked on the host. |
...
manager sees the Terminated but does nothing with it: it
**doesn't re-spawn**, and it **doesn't release the lease** — a held
lease stays held, and the manager's `child` reference is left
pointing at the now-dead singleton.  The singleton is gone until the
next leader change.
