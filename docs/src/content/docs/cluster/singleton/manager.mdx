...
<Aside type="caution" title="A role nobody carries hosts nowhere">
  ```ts
  StartSingletonOptions.create<JobCommand>().withRole('control-plane');
  ```
  The manager hosts only when this node is **both** the cluster leader
  **and** carries the role.  If the elected leader does not carry it,
  no node hosts the singleton at all — unlike Akka, which picks the
  oldest member *with* the role.  Give the role to the nodes that sort
  earliest, or leave it unset, until that is fixed.
</Aside>
