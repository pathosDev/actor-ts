...
### When the acquire hangs

The interesting case is not "the lease said no" but "the lease
said nothing".  If `acquire()` has not resolved within
`acquireTimeoutMs` (5 s by default), the strategy stops waiting
for it — and then has to assume the attempt may still succeed on
the wire, leaving the lease claimed by a node that no longer
believes it won.  Three rules cover that:
