...
Every option in the table above except the last four is also an
`actor-ts.worker-cluster` key, so a deployment can retune the mesh without a
rebuild:
...
duration literal instead.  `bootstrap`, `initData`, `backend` and
`onWorkerPermanentlyDown` have no keys — the first two are per-call identity,
the last two live objects a file cannot express.
