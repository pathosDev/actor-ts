...
The supervisor accepts arbitrary user messages — that is the whole
point, it forwards them to the child — so a `Terminated` naming the
current child is something anything holding the supervisor's ref could
send it.  Acting on that would retire a live child, bump the backoff
counter, and spawn a replacement beside a `child-1` nobody stopped and
nobody watches any more.

Two things stop it.  The runtime
[refuses a `Terminated` it did not emit](/fundamentals/death-watch/)
before the supervisor ever sees it.  And the supervisor asks the
child's own cell whether it has actually terminated before it retires
it, so even a genuine notification about a running actor is declined
with a warning rather than obeyed.  If a predecessor is somehow still
alive when its replacement is about to be spawned, it is stopped rather
than left orphaned.

None of this is configurable, and none of it costs anything on the
ordinary path: a child that really died reports `terminated` before its
watchers hear about it, so the check is a boolean read.

## Configuration

The `BackoffSupervisorOptionsType<T>` shape — `child` is the only
required field:
...
The arrows name the [`actor-ts.backoff-supervisor`](/reference/configuration/#actor-tsbackoff-supervisor)
leaf that sets the field process-wide.  The three layers are the usual
ones and they apply **per field**: an option written here wins, a
configured leaf fills in what it left unset, and the built-in default
covers the rest.  So a supervisor written as
`BackoffSupervisor.factory({ child: Flaky })` takes its whole pacing from
the config file, and one that names `minBackoff` still takes
`max-backoff` from there.

Or the builder, which records only the fields you set — so it never
competes with config either:
...
The merge runs in the supervisor's `preStart` — the first moment
`system.config` is reachable — so an out-of-range value fails the
supervisor's *start*, not the call that built it.  It is the same rule
either way: `min-backoff = 0s` in a config file is refused exactly as
`minBackoff: 0` in code is.

`drainGraceMs` and `childName` have no config leaf on purpose.  The
first's default is derived from `minBackoff`, so publishing `50ms` would
freeze it for a supervisor with a shorter window; the second is
per-call-site identity, where one process-wide value would be actively
wrong.
...
**When the stash fills.**  A backoff window longer than the traffic can
wait through eventually pushes the stash past `maxStashSize`, and the
oldest buffered message makes room for the newest.  That message is not
discarded silently: it becomes a
[dead letter](/operations/dead-letters/), carrying its payload and the
sender that would have received the reply, so an overflow is
reconstructible afterwards instead of being a gap in the trace.

The accompanying warning is **aggregated**, not one line per message — it
fires on the first eviction and again at each doubling, and carries the
running total.  A flood against a supervisor stuck in a long backoff would
otherwise turn a message flood into a log flood at exactly the same rate.
...
fibonacci, custom.  `minBackoff` / `maxBackoff` still apply (they are
advisory caps; the framework uses them for the `resetCounter` heuristic
and for the drain grace), but the `policy` controls the actual delay
computation — and with it the jitter, so `randomFactor` is inert for
such a supervisor whether it was set in code or configured.
