...
| `ws` | `WebSocketClientActor` (outbound client on Node < 22) + server-side WebSocket upgrades on the Express backend. |
...
<Aside type="caution" title="Older Node versions">
  ...
  the `WebSocketClientActor` needs the `ws` package (Node 22+ has it
  natively), and zstd compression needs `fzstd` (Node 22.15+ has it
  natively).  Server-side `websocket()` routes need `@fastify/websocket`
  on the Fastify backend or `@hono/node-ws` on the Hono backend when
  running on Node.  Node ≤ 19 is not supported.
</Aside>
