...
| `GET /ready` | Readiness — bereit für Traffic (Framework- + eigene Checks)? |
| `GET /cluster/members` | Liste der Cluster-Mitglieder (wenn ein Cluster übergeben wird). |
| `GET /cluster/shards?type=<name>` | Shard-Verteilung für einen sharded Typ (404, solange dieser Node keine Region oder Proxy dafür gestartet hat). |
