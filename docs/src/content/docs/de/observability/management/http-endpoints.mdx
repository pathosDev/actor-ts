...
Das `checks`-Array enthält jeden Readiness-Check auf der Registry
...
### Die beiden Probe-Pfade verschieben

`/health` und `/ready` sind Defaults, keine festen Routen.  Beide
sind ein einzelnes Pfadsegment und im Code wie aus einer
Config-Datei setzbar:
...
Ein Wert mit `/` wird beim Bauen der Routen mit einem
`OptionsError` abgelehnt, denn `path()` entfernt nur
*umschließende* Slashes — aus `"k8s/ready"` würde ein Segment mit
einem Slash darin, das auf keine URL passt und den Endpunkt
lautlos verschwinden lässt.  Wie die Segmente auch heißen: halte
die Probe-Pfade in K8s im Gleichschritt damit, ein Manifest wird
von nichts gegengeprüft.
...
- `version` zählt *Broadcasts* des Coordinators, keine
  Shard-Zuweisungen — eine Erhöhung pro Publish, und ein Publish
  fasst einen Schwung Verschiebungen zusammen.  Vergleiche den Wert
  über zwei Abfragen, um „die Karte hat sich bewegt“ von „die Karte
  ist unverändert“ zu unterscheiden.
- `takenAt` und `leader` stempelt der antwortende Node beim Eintreffen
  der Karte, zwei Nodes können also dieselbe `version` mit leicht
  verschiedenen Zeitstempeln melden.
- `regions[].key` ist `<Adresse>|<Region-Pfad>`, derselbe Key, den
  `shardHome[].regionKey` verwendet.  Eine Region, die nichts hostet,
  steht trotzdem in der Liste, mit leerem `shards` — „registriert und
  ohne Shards“ ist die interessante Hälfte einer Rebalance-Frage.
- Eine **200 mit leerem `shardHome`** ist normal und kein Fehler:
  `regions` füllt sich, sobald sich Regionen registrieren, während ein
  Shard erst dann ein Zuhause bekommt, wenn eine Entity darin
  adressiert wird.
...
die [prom-client-Bridge](/de/observability/metrics/prom-client-adapter/)
...
Ein `200` mit leerem Body wäre schlimmer als der Fehler: eine leere
...
`managementRoutes(system, cluster, options)` gibt die Basis-Routen
...
const routes = managementRoutes(system, cluster, {
  ...
  ipAllowlist: IpAllowlist({
    ...
    // Hinter einem Reverse-Proxy: benenne die Proxys — die
    // Client-Adresse wird dann vom Socket-Ende der
    // Forwarded-Kette nach innen aufgelöst.  Ohne das liest der
    // Default den Socket-Peer, und der ist hinter einem Proxy
    // der Proxy.
