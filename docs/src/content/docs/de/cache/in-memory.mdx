...
## Eviction ist blind dafür, was ein Eintrag schützt

Aktualität ist das *einzige* Kriterium.  Ein Rate-Limit-Zähler und ein
Idempotency-Record werden genauso bereitwillig evictet wie ein
gecachter Response-Body — und das Wegwerfen entwertet stillschweigend
die Garantie, die der Eintrag getragen hat:
...
Beides hinterlässt keine Spur: der Eintrag ist einfach weg, weit
innerhalb seiner TTL.

Die Exposition entsteht durch das **Teilen einer Cache-Instanz**
zwischen einem Consumer, der eine Garantie trägt, und einem, dessen
Key-Raum ein Aufrufer aufzählen kann.  Ein Response-Cache mit einem Key
aus einem Pfadparameter und ein `idempotent`-Wrapper mit einem Key aus
einem Client-Header fallen beide in die zweite Kategorie — wer einen
davon variiert, wälzt die gesamte Map um.  Mit der Default-Schranke von
10 000 Einträgen und einem großzügigen Limit von 100 Requests pro
Sekunde dauert das rund 100 Sekunden — vollständig *innerhalb* des
dokumentierten Rate-Limits.

Die Abhilfe ist ein Cache pro Consumer:
...
erreicht die anderen also nicht.  Dimensioniere `maxEntries` jeweils
allein für den Key-Raum dieses Consumers.
...
IPv6-`/64` erzeugt beliebig viele Rate-Limit-Keys.  Wo das zählt,
hinterlege den sicherheitsrelevanten Consumer mit Redis (den du separat
dimensionierst und überwachst) statt mit einer prozesslokalen LRU.
