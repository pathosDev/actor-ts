...
bei der System-Erstellung via `withPersistence` verdrahtet:
...
Eingebaute Snapshot Stores.  Die dritte Spalte ist die
`persistenceOptionSupport`-Deklaration des Stores selbst — ob er auf
die Hooks `compression()` / `encryption()` / `integrity()` reagiert
oder sie bloß entgegennimmt:

| Store | Wann | Honoriert die drei Hooks |
| --- | --- | --- |
| `InMemorySnapshotStore` | Tests, Dev. | Nein. |
| `SqliteSnapshotStore` | Single-Node-Produktion. | Nein. |
| `PostgresSnapshotStore` | PostgreSQL-gestützte Produktion. | Nein. |
| `MariaDbSnapshotStore` | MariaDB / MySQL-gestützte Produktion. | Nein. |
| `LibSqlSnapshotStore` | libSQL / Turso — entferntes SQLite über HTTP. | Nein. |
| `MsSqlSnapshotStore` | Microsoft SQL Server-gestützte Produktion. | Nein. |
| `MongoSnapshotStore` | MongoDB-gestützte Produktion. | Nein. |
| `DynamoDbSnapshotStore` | DynamoDB-gestützte Produktion. | Nein. |
| `D1SnapshotStore` | Cloudflare D1 — SQLite am Edge. | Nein. |
| `CassandraSnapshotStore` | Verteilt — Cassandra / ScyllaDB. | Nein. |
| `ObjectStorageSnapshotStore` | Filesystem / S3 mit optionaler Verschlüsselung. | Ja — alle drei, beim Schreiben und beim Lesen. |
| `CachedSnapshotStore` | Wickelt einen anderen Store mit einem Read-Through-Cache ein. | Was der eingewickelte Store deklariert. |

Ein `Nein` in dieser Spalte ist durchgesetzt, nicht nur dokumentiert.
`encryption()` oder `integrity()` auf einem Actor zu setzen, der von
einem dieser Stores getragen wird, **verweigert den Actor beim Start**
mit einem `UnsupportedPersistenceOptionError`, der Store und Feld
benennt — das sind Sicherheitskontrollen, und Klartext zu schreiben,
statt die Lücke zu melden, ist genau der Fehler, den die Verweigerung
verhindern soll.  `compression()` ist ein Performance-Hinweis, also
gibt es eine einzelne Warnung mit dem Store-Namen und der Snapshot
wird unkomprimiert geschrieben.  `{ mode: 'none' }` wird überall
akzeptiert: So sagst du "bewusst ungeschützt".

Ein Store, der gar nichts deklariert — eine Fremdimplementierung aus
der Zeit vor der Deklaration — gilt als *unbekannt* und wird nie
verweigert, aus derselben Überlegung wie bei `storageLocality`: Ein
Store darf nicht an einer Vorgabe gemessen werden, die er nicht
gewählt hat.
...
`Uint8Array` in einem Snapshot als echte Instanzen recovern —
siehe
[was Events und State enthalten dürfen](/de/persistence/persistent-actor/#was-events-und-state-enthalten-dürfen).

## Aufbewahrung ist Best-Effort, der Schreibvorgang nicht

Stores, die die Aufbewahrung begrenzen (`keepN`), räumen bei jedem
Speichern auf.  Dieser Aufräumdurchlauf ist **auf jedem Backend
Best-Effort**: Sobald der Snapshot selbst dauerhaft ist, ist `save`
erfolgreich, und ein danach fehlschlagendes Aufräumen wird
geschluckt statt nach oben gereicht.

Die beiden Hälften bedeuten im Fehlerfall Gegensätzliches, weshalb
sie nicht gleich gemeldet werden:

- Ein fehlgeschlagener **Schreibvorgang** heißt, dass dein Snapshot
  nicht existiert.  Ein erneuter Versuch ist die richtige Reaktion,
  und `save` schlägt fehl, damit du das tun kannst.
- Ein fehlgeschlagenes **Aufräumen** heißt, dass eine Zeile zu viel
  existiert.  Das ist harmlos, `loadLatest` sieht sie nicht, und
  das nächste Speichern versucht es erneut.

Das Zweite wie das Erste zu melden würde einem Aufrufer sagen, er
solle einen bereits erfolgreichen Schreibvorgang wiederholen — und
ein Actor, der einen Snapshot-Fehler als fatal behandelt, würde an
einem Aufräumfehler sterben.  Wenn du einen eigenen `SnapshotStore`
implementierst, führe das Aufräumen außerhalb der Fehlerbehandlung
des Schreibvorgangs aus.
