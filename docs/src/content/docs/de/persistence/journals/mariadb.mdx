...
<Aside type="caution" title="Tags vergleichen im Index case-insensitiv">
  `tag` ist ohne explizites `COLLATE` deklariert und erbt damit den
  Server-Standard — auf einer Standardinstallation case-insensitiv.  Der
  Index liefert deshalb auch Zeilen für Tags, die sich nur in der
  Groß-/Kleinschreibung unterscheiden.  Die Ergebnisse bleiben korrekt:
  `MariaDbQuery` prüft jede Zeile erneut gegen die Tag-Liste des Events
  selbst, und die vergleicht exakt.  Der Preis ist ein weniger selektiver
  Vorfilter als bei derselben Query auf Postgres.  Deklariere die Spalte
  als `COLLATE utf8mb4_bin`, wenn du das Schema vorab anlegst und den
  Index die ganze Arbeit machen lassen willst.
</Aside>
