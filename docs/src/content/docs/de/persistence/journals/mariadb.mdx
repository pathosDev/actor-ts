...
const { durableStateStore } = registerMariaDbPlugins(ext, RegisterMariaDbPluginsOptions.create()
  .withJournal(MariaDbJournalOptions.create().withPoolConfig(conn))
  .withSnapshotStore(MariaDbSnapshotStoreOptions.create().withPoolConfig(conn).withKeepN(3))
  .withDurableStateStore(MariaDbDurableStateStoreOptions.create().withPoolConfig(conn)));
