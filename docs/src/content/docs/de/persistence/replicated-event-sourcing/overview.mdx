...
Voraussetzung.  Daraus folgen zwei Eigenschaften, und beide sind
...
<Aside type="caution" title="Die Autorschaft ist noch nicht authentifiziert">
  ```ts
  // A member can publish an event attributed to another replica.
  ```
  Nichts bindet das `replica`-Feld eines Envelopes an den Node, der ihn
  gesendet hat.  Ein Cluster-Mitglied kann also ein Event verfassen, das
  jede Replica unter der Id eines Peers faltet und journalisiert.
  *Unterdrücken* kann es die echten Events eines Peers nicht — das ist
  der Teil, den die Entropie schließt —, aber eine replizierte Entity
  ist nur so vertrauenswürdig wie die Zugangskontrolle des Clusters
  selbst.  Betreibe Replicated ES hinter mTLS und behandle
  Cluster-Mitgliedschaft als Privileg.
</Aside>
