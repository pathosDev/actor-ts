...
| `undefined` in Arrays / `Set` / `Map`-Einträgen | Wird `null`, wie bei `JSON.stringify`. |
| `NaN`, `Infinity` | Werden `null` (Plain-JSON-Grenze). |
...
| `RegExp`, `Error` | Degradieren zu `{}` / Teildaten — in Payloads vermeiden. |
| Funktionen, Symbols, zirkuläre Referenzen | **Werfen zur Persist-Zeit** (`SerializationError` — laut, statt die gespeicherte Row zu korrumpieren). |
