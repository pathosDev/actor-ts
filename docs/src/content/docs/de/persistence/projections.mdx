...
import { SqliteQuery, DurableStateOffsetStore, PostgresDurableStateStore } from 'actor-ts';
...
const byTagProjectionOptions = ByTagProjectionOptions.create<AccountEvent>()
  ...
  .withOffsetStore(new DurableStateOffsetStore(new PostgresDurableStateStore()))
...
import { PostgresDurableStateStore } from 'actor-ts';

// Default (verloren beim Neustart):
new InMemoryOffsetStore();

// Produktion (durable — nutzt ein beliebiges DurableStateStore-Backend wieder):
new DurableStateOffsetStore(new PostgresDurableStateStore());
```
...
- `DurableStateOffsetStore` — persistiert Offsets über einen
  beliebigen `DurableStateStore` (Postgres, MariaDB, Object-Storage),
  sodass du ein bereits laufendes durables Backend wiederverwendest.
