...
Den Integritätsschlüssel selbst rotiert der Sweep **nicht**.  Auf
der Leitung ist nirgends festgehalten, welcher Integritätsschlüssel
einen Body versiegelt hat; ein halb gesweepter Korpus wäre also
nicht von einem manipulierten zu unterscheiden — rolle einen neuen
Integritätsschlüssel stattdessen über das Read-then-Write-Fenster
des Stores aus.
...
- **Exakt `<keyPrefix><persistenceId>/<leaf>`.** Das ist das
  Layout beider eingebauter Stores — `<pid>/<seq>.json` für
  Snapshots, `<pid>/state.json` für Durable State. Ein Key mit einer
  zusätzlichen Ebene wird verweigert statt auf sein erstes Segment
  reduziert: eine `persistenceId` mit `/` darin oder ein `keyPrefix`,
  der kürzer ist als der `prefix` des Stores, ergäbe sonst ein
