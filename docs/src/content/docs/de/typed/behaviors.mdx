...
const ticker = Behaviors.receive<Message>((ctx, msg) => {
...
```ts
const counter = (n: number): Behavior<Message> => Behaviors.receive<Message>((ctx, msg) => {
  switch (msg.kind) {
...
```ts
const counter = (n: number): Behavior<Message> => Behaviors.receiveMessage<Message>((msg) => {
  if (msg.kind === 'increment') return counter(n + 1);
...
```ts
const myActor = Behaviors.setup<Message>((ctx) => {
...
const ready = Behaviors.receive<Message>((ctx, msg) => {
