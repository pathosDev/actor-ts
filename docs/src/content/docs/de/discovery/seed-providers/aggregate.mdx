...
nächsten; ein Fehler triggert ebenfalls einen Fallback (der
Fehler wird an einen optionalen Log-Callback übergeben —
standardmäßig ein No-op).
...
Ein 403 von der K8s-API → übersprungen → fällt durch zum Config
Provider.  Das macht Aggregate zum richtigen Werkzeug für
"dieser Code-Pfad läuft überall" — das K8s-Lookup wird einfach
zum No-op, wo K8s nicht verfügbar ist.

`AggregateSeedProvider` nimmt ein **optionales zweites
Konstruktor-Argument** — einen Log-Callback `(message: string,
err?: unknown) => void`.  Er ist standardmäßig ein No-op, sodass
Provider-Fehler ohne übergebenen Callback **stillschweigend
verschluckt** werden.  Übergib eine Log-Funktion, um Sichtbarkeit
zu bekommen, welche Provider es probiert / nicht geschafft haben:
