...
new AggregateSeedProvider([
  ...
  new ConfigSeedProvider(
    ConfigSeedProviderOptions.create().withSeeds(['10.0.0.1:2552']).withSystemName('my-app'),
  ),
]);
```
...
<Aside type="caution" title="Latenzen stapeln sich">
  ...
  new AggregateSeedProvider([
    new DnsSeedProvider(
      DnsSeedProviderOptions.create().withHostname('slow-dns.example').withSystemName('my-app').withUseSrv(),
    ),   // 30s Timeout
    new ConfigSeedProvider(
      ConfigSeedProviderOptions.create().withSeeds(['fallback:2552']).withSystemName('my-app'),
    ),
  ]);
  ```
