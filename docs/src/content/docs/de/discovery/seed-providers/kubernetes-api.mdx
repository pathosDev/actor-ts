...
type KubernetesApiSeedProviderOptionsType = {
  ...
  systemName:      string;
  port:            number;
  requestTimeoutMs?: number;                 // Obergrenze für den API-Request, Default 10 000
...
| `requestTimeoutMs` | Obergrenze für den einen Request, den der In-Cluster-Fetcher macht, in ms.  Default 10 000.  Danach lehnt `lookup()` ab, sodass die Discovery-Leiter zur nächsten Sprosse fällt, statt an einem API-Server zu hängen, der annimmt und nie antwortet.  HOCON: `actor-ts.discovery.kubernetes.request-timeout`. |
...
Für Pods, die in-cluster laufen, sind nur die letzten vier Felder
