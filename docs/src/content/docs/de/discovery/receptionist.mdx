...
gewatcht, siehe
[Subscriptions sind begrenzt und gewatcht](#subscriptions-sind-begrenzt-und-gewatcht).)
...
Zwei Mechanismen halten die Subscriber-Menge davon ab, unbegrenzt
zu wachsen.
...
**Obergrenzen.** Zwei Limits begrenzen den Rest, die lebenden
Subscriber:
...
Die beiden zählen Verschiedenes, und die Namen sagen was.
`maxSubscribersPerKey` zählt Subscriber an einem Key.
`maxSubscriptionsTotal` zählt **Key/Subscriber-Paare** über alle Keys, ein
Subscriber mit drei Keys verbraucht davon also drei — Paare sind das, was den
Speicher begrenzt, denn wie viele Keys ein einzelner Subscriber halten darf,
beschränkt nichts.

Ein `Subscribe` über einer der beiden Grenzen wird **abgelehnt,
...
Die Antwort ist wichtiger, als sie aussieht: ein stillschweigend
