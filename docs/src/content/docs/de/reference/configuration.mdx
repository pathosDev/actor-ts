...
Vier Felder haben bewusst keinen Key: `bootstrap` und `initData` sind
Identität des einzelnen Aufrufs, und `backend` sowie `onWorkerPermanentlyDown`
sind lebende Objekte, die eine Config-Datei nicht ausdrücken kann.
