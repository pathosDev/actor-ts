...
actor-ts {
  ...
  sharding {
    ...
    passivation-idle    = 0ms   # 0 deaktiviert die Auto-Passivierung
...
| `http.shutdown-grace-period` | `5s` | Wie lange `unbind()` in-flight Requests leerlaufen lässt, bevor Verbindungen erzwungen geschlossen werden. |
...
Die Grace Period ist eine **Obergrenze, keine Wartezeit**: `unbind()`
kehrt zurück, sobald der Server tatsächlich geschlossen ist, und wartet
das Fenster nur aus, wenn noch Requests laufen.  Ein Aufrufer, der einen
eigenen Wert übergibt — `binding.unbind(1_000)` — überschreibt sie.
