...
Beide Caps gibt es, weil die Mailbox des Replikators schon eines hat — und
ihres hier schlecht ausgeht.  Der Replikator läuft auf der bounded
Default-Mailbox (10 000 Einträge, `drop-head`), bei Sättigung wird also der
*älteste* Envelope verworfen — und ein wartender `ddata-update` trägt
`resolve` / `reject` des Aufrufers in sich.  Ihn zu verwerfen lässt
`updateAsync` auf ein Promise warten, das nie settlet, ohne jeden Logeintrag.
Deutlich unter der Mailbox-Kapazität gesetzt, macht
`max-pending-quorum-requests` aus diesem stillen Drop einen expliziten
Fehler, der den Knopf beim Namen nennt; auf 10 000 gesetzt griffe es nie
zuerst und würde nichts umwandeln.  Siehe
