...
- Bei **Broker-Aktoren** (MQTT, Kafka, WebSocket-Client, …) wird der
  ...
  weiterhin als `BrokerSettingsError`, und nur *vorhandene, aber
...
falscher Typ, fehlender Pfad) und `BrokerSettingsError` (ein
...
try {
  const mqttOptions = MqttOptions.create()
    .withBrokerUrl('mqtt://broker:1883')
    .withQos(7 as MqttQos);          // ungültig
  system.spawn(Props.create(() => new FeedClient(mqttOptions)), 'feed');
} catch (e) {
