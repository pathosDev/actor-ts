...
| Key | Default | Zweck |
| --- | --- | --- |
| `dead-letters.store` | `"off"` | `off` = nur publishen, nichts aufbewahren / `memory` = ein begrenzter Ring / `persistent` = zusätzlich ins Journal schreiben, sodass die Queue einen Neustart übersteht. |
| `dead-letters.max-entries` | `1000` | Aufbewahrte Letters, bevor das älteste verdrängt wird. |
...
Siehe [Dead Letters](/de/operations/dead-letters/) dafür, was die Queue
enthält und wie man daraus erneut zustellt.
