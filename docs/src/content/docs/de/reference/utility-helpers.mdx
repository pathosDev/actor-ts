...
<Aside type="caution" title="Nicht über einen eigenen CBOR-Serializer">
  Ein Store, der mit `withSerializer(new CborSerializer())` konfiguriert ist,
  trägt sie nicht — dieser Codec trägt auch `Map` und `Set` nicht. Der
  Default-Codec, den jeder Store benutzt, solange du ihn nicht änderst, tut es.
</Aside>
