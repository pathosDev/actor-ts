...
<Aside type="caution" title="Renewal-Fehler = Lease sofort verloren">
  ...
  fehlgeschlagene Renew — ein verlorenes Race, ein `409`/`404`
  oder ein beliebiger transienter Backend-Fehler — feuert
  `onLost` und verliert den Lease sofort.  `acquireRetries` gilt
  nur für `acquire()`, nie für den Renewal-Loop; behandle daher
  jedes `onLost` als echten Eigentumswechsel.
</Aside>
