...
| `operationTimeoutMs` | `10000` | Obergrenze für einen einzelnen Request an den API-Server — Wall-Clock über den ganzen Austausch, ein TLS-Handshake, den der Server nie abschließt, ist also mit begrenzt. |
...
also legitim über zwei Takte laufen kann.  Dieser Timeout ist eine
Wall-Clock-Frist über ein Abort-Signal, kein Socket-Timeout: Bun scharf
den Socket-Timeout nie, solange ein TLS-Handshake aussteht, und ein
Halter, dessen API-Server mitten im Handshake stehen blieb, hing früher
mit seinem Renewal an einem einzigen Request fest, statt die Lease zu
verlieren (#1529).  Ein Takt, der einen noch
