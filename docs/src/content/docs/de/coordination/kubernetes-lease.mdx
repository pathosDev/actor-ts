...
| `apiServerUrl` | in-cluster | Die URL des K8s-API-Servers — Default `https://kubernetes.default.svc`. |
| `authToken` | in-cluster | Das Service-Account-Token des Pods — Default `/var/run/secrets/kubernetes.io/serviceaccount/token`. |
| `caCert` | in-cluster | PEM-kodiertes CA-Zertifikat für das TLS des API-Servers — Default `/var/run/secrets/kubernetes.io/serviceaccount/ca.crt`. |
...
`onLost` — es gibt kein Retry-Budget innerhalb der Schleife:
