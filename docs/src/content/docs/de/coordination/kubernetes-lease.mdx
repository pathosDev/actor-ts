...
`onLost` — es gibt kein Retry-Budget innerhalb der Schleife, mit einer
Ausnahme:

- **Transient (5xx, connection refused, Timeout)** → `onLost` feuert.
- **CAS-Conflict (409) oder 404** → ein anderer Halter hat übernommen,
  oder das Objekt wurde gelöscht; `onLost` feuert.
