...
- **Eine Mailbox weiß nicht, was sie verwirft.**  `drop-head` passt zu
  ...
  Einer davon, das `Terminated` der Death Watch, ist inzwischen für jede
  Policy unerreichbar: es nimmt eine Spur, die keine Schranke abwerfen
  kann (siehe [Death Watch](/de/fundamentals/death-watch/) und *Eine
  eigene Mailbox* weiter unten).  Der Rest ist Anwendungsverkehr, und
  nur du weißt, welcher davon verzichtbar ist.
...
Eine Queue, die abwirft, hat eines, das sie nicht abwerfen darf.  Das
Framework kündigt einen Tod genau einmal an — siehe
[Death Watch](/de/fundamentals/death-watch/) — deshalb kommt ein
`Terminated` über `enqueueSignal` statt über `enqueue`, und der Vertrag
lautet: *reihe es ein, was deine Schranke auch sagt*:
