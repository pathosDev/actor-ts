...
- **Der Typparameter `<Cmd>`** schränkt ein, welche Nachrichten dieser
...
- **`override onReceive(cmd: Cmd): void`** ist der einzige Pflicht-Override.
...
class Echo extends Actor<{ text: string }> {
  override onReceive(msg: { text: string }): void {
    ...
    //     `context.actorOf(...)` für Kinder, `context.watch(...)` für
...
Actors können Kinder haben.  Verwende `context.actorOf` aus dem
...
class Parent extends Actor<{ kind: 'add-child'; name: string }> {
  override onReceive(msg: { kind: 'add-child'; name: string }): void {
    const child = this.context.actorOf(
...
class DbWriterFanout extends Actor<{ row: Row }> {
  override onReceive(msg: { row: Row }): void {
    ...
    this.context.actorOf(Props.create(() =>
