...
coordinatedShutdown.installProcessHooks();   // SIGTERM/SIGINT → coordinatedShutdown.run(ProcessTerminateReason)
```
...
```ts
coordinatedShutdown.addTask(Phases.ClusterLeave, 'gossip-leave', async (reason) => {
...
```ts
coordinatedShutdown.installProcessHooks();
// Oder: coordinatedShutdown.installProcessHooks(['SIGTERM', 'SIGINT', 'SIGUSR2']);
```

Das hängt Handler an, die `coordinatedShutdown.run(new ProcessTerminateReason(signal))`
...
Wenn SIGTERM ankommt, feuert der Hook `coordinatedShutdown.run(...)`, die Pipeline
