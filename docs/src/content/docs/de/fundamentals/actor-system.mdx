...
Wenn du `system.spawn(props, name)` aufrufst, wird der Actor unter
...
3. `/system` stoppen — Framework-Internas wickeln sich ab.  Das beginnt
   erst, wenn `/user` vollständig geleert ist, sodass ein `postStop` aus
   Schritt 2 noch mit den Sharding-, PubSub- oder Delivery-Actors reden
