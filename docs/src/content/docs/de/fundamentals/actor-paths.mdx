...
2. **Der Name sollte "wie ein Pfadsegment aussehen."**  Keine
   Slashes, keine führenden Punkte, keine Leerzeichen.  Das Framework
   erzwingt das **nicht** — es prüft nur die Eindeutigkeit unter
   Geschwistern (Regel 1) und dass der Parent nicht bereits gestoppt
   ist —, sodass ein Name mit einem Slash stillschweigend das
   Pfad-Parsing und `ActorSelection` bricht.  Validiere Segmente
   selbst (siehe den Warnhinweis unten).
