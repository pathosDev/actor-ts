...
1. **Namen müssen unter Geschwistern eindeutig sein.**  Zweimal
   `context.actorOf(props, 'bar')` auf demselben Parent wirft einen
...
- **`selection.tell(msg)`** — einmal auflösen, sofort liefern, wenn
...
- **`(await selection.resolveOne(timeout)).tell(msg)`** — warte, bis
...
| Pfad | Was dort lebt |
| --- | --- |
| `/user` | Die Top-Level-Actors deiner Anwendung (alles, was `system.actorOf(...)` erzeugt). |
