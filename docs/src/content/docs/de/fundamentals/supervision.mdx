...
5. Die Mailbox wird **beibehalten**.  Die nächste Nachricht (die
   *nach* der fehlgeschlagenen) wird auf der neuen Instanz
   verarbeitet.
6. Die **Kinder bleiben erhalten**.  Sie hängen an der Cell, die die
   ersetzte Instanz überlebt — sie behalten also ihren eigenen
   Zustand und ihre Mailbox über den Restart des Parents hinweg.
...
<Aside type="caution" title="Ein benanntes Kind in `preStart` spawnen">
  Punkt 6 hat eine scharfe Kante.  Weil Schritt 4 `preStart` erneut
  ausführt, während die Kinder der vorigen Inkarnation noch in der
  Child-Map stehen, wirft ein Actor, der in `preStart` ein
  **benanntes** Kind spawnt, beim ersten Restart
  `Child name '<name>' is not unique`.

  Sind die Kinder entbehrlich, spawne sie anonym — die generierten
  Namen kollidieren nie:
  ...
  Soll der Restart sie neu aufbauen, stoppe sie selbst:
  ...
  Beides nicht zu tun ist ebenfalls in Ordnung — überlebende Kinder
  sind oft genau das, was man will, und ein Parent, der neu startet,
  ohne seinen Teilbaum anzufassen, ist das billigere Ergebnis.
</Aside>
