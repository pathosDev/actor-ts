...
| `{ from: 3 }` | `from` Replikas haben geACKt (begrenzt auf `[1, N]`). |
...
Default-Timeout ist feste 5 Sekunden (`DEFAULT_ASK_TIMEOUT_MS`),
unabhängig vom Gossip-Intervall.  Es bleibt auch dann fest, wenn
`actor-ts.actor.ask-timeout` konfiguriert ist: Dieser Key verschiebt den
Default von `ActorRef.ask`, während die Quorum-Frist vom
Distributed-Data-Handle aufgelöst wird, das bislang keine eigene
Konfiguration liest.  Sie einstellbar zu machen gehört in den Block
`actor-ts.distributed-data` — verfolgt als #856.  Pro Call
überschreiben, wenn:
...
## Die Pending-Menge begrenzen

Jeder Quorum-Call, der sich nicht schon mit dem Self-ACK auflösen kann,
parkt einen Eintrag — ein Promise, einen Timer, die Menge der Peers, auf
die er wartet — bis genug Antworten da sind oder die Deadline abläuft.
Zwei Settings begrenzen diese Menge, im Code oder über
[`actor-ts.distributed-data`](/de/reference/configuration/#actor-tsdistributed-data):
...
| --- | --- | --- |
| `maxPendingQuorumRequests` | `1000` | Maximal offene Quorum-Requests gleichzeitig.  `updateAsync` und `getAsync` teilen sich das Budget.  `0` deaktiviert es. |
| `maxQuorumTimeout` | `30000` | Obergrenze in ms für das `timeoutMs` eines Calls; ein größerer Wert wird heruntergeschnitten.  `0` deaktiviert sie. |

Über dem Cap lehnt der Call sofort mit einem Fehler ab, der das Setting
beim Namen nennt — er wird **nicht** eingereiht und wartet **nicht**:
...
Wie beim Timeout macht eine Ablehnung den **lokalen Write nicht
rückgängig** — der Wert ist vor dem Quorum angewandt und gossipt normal
weiter.

<Aside type="tip" title="Was der Cap tatsächlich begrenzt">
  Nicht die Queue — die Mailbox des Replikators ist unbounded wie die jedes
  anderen Aktors, es wird also nie ein `updateAsync`-Envelope auf dem Weg
  hinein verworfen.  Was sich ansammelt, ist die **Menge der offenen
  Requests**: jeder hält ein Promise, einen Timer und ein Ziel-Set am Leben,
  bis seine Deadline abläuft.  Ohne Cap lässt eine Partition, die Quoren nicht
  mehr zustande kommen lässt, alle drei anwachsen — und am Ende laufen sie
  gemeinsam als Timeout-Sturm ab.  Der Cap macht daraus sofortige Fehler, die
  du an der Aufrufstelle fangen kannst, solange der Request noch dem Aufrufer
  zuzuordnen ist, der ihn gestellt hat.
</Aside>

## In Produktion beobachten

Mit aktivierten [Metriken](/de/observability/metrics/core-metrics/) veröffentlicht der
Replikator vier Serien — die Label-Mengen sind fest, sie kosten also eine
Handvoll Serien und nicht mehr:
...
Alarmierungswürdig ist vor allem das letzte Paar.  Ein steigendes
`..._rejected_total` heißt, dass Aufrufer dem Cap davonlaufen — entweder
ein durchgegangener Producer oder ein für die Last zu niedriges Cap.  Ein
steigendes `..._dropped_values_total` heißt, dass ein Peer CRDT-Payloads
schickt, die diese Replika nicht dekodieren kann — ein kaputter oder
feindlicher Peer, kein Tuning-Problem.
