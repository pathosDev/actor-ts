...
<Aside type="tip" title="Warum der Default 1000 ist und nicht 10 000">
  Der Replikator läuft auf der bounded Default-Mailbox — 10 000 Einträge,
  `drop-head`.  Bei Sättigung verwirft diese Policy den **ältesten**
  wartenden Envelope, und ein wartender `updateAsync`-Envelope nimmt dein
  `resolve` und `reject` mit: das Promise, auf das du wartest, kann dann nie
  mehr settlen, und geloggt wird nichts.  Ein Cap von 10 000 würde nie
  erreicht, bevor die Mailbox zuerst dort ankommt — es würde also nichts
  umwandeln.  Eine Größenordnung darunter gesetzt, besteht der ganze Wert
  des Caps darin, aus diesem stillen Drop einen expliziten, fangbaren
  Fehler zu machen.
</Aside>
