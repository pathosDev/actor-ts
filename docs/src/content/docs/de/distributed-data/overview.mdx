...
const distributedDataOptions = DistributedDataOptions.create().withDurableStore(
    new DurableDistributedDataStore({
      ...
      keys: ['hits', 'config'],   // nur diese Keys werden persistiert
    }),
  );
...
Die Durable-Schicht persistiert Änderungen auf Disk (oder
wohin auch immer der Durable-State-Store schreibt); beim Neustart
wird der Stand der Replika wiederhergestellt, bevor sie Gossip
beitritt.
