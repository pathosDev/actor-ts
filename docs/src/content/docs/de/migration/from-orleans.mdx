...
```ts
// actor-ts:
import { ClusterSharding } from 'actor-ts/cluster';
...
Actor, der beim ersten Eingang spawnt.  Anders als ein
Orleans-Grain deaktiviert er sich *nicht* von selbst: es gibt kein
Standard-Idle-Fenster, eine Entity bleibt also resident, bis sie
jemand passiviert (siehe unten).
