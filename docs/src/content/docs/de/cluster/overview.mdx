...
| Einstellung | Zweck |
| --- | --- |
| `host` + `port` | Das Interface, an das dieser Node **bindet**. `0.0.0.0` heißt "jedes Interface" und ist der Default. |
| `advertisedHost` | Die Adresse, die Peers **wählen** — die Identität dieses Nodes. Ungesetzt wird sie aus `host` abgeleitet, sofern der routebar ist, dann aus `CLUSTER_HOST` / `POD_IP` / `HOSTNAME`, dann Loopback. |
