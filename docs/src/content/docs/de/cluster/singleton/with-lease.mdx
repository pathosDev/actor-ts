...
```ts
import { ClusterSingletonManager, ClusterSingletonManagerOptions, KubernetesLease, KubernetesLeaseOptions } from 'actor-ts';
...
const clusterSingletonManagerOptions = ClusterSingletonManagerOptions.create()
  ...
  .withSingletonProps(singletonProps)
  .withLease(lease)                        // ← optionale Lease
  .withAcquireRetryIntervalMs(5_000);
ClusterSingletonManager.props(
  clusterSingletonManagerOptions,      // Default — Retry nach fehlgeschlagenem Acquire
);
```
