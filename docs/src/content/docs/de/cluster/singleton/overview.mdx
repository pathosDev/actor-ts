...
```ts
import { Actor, ActorSystem, Cluster, ClusterOptions, Props } from 'actor-ts';
...
// Auf jedem Node: starte das Singleton über die ClusterSingleton-Extension.
// Ein Aufruf spawnt den Manager dieses Nodes und baut den Proxy; nur der
// Manager des Leaders konstruiert tatsächlich den JobScheduler.
const startSingletonOptions = StartSingletonOptions.create<JobCommand>()
  .withTypeName('job-scheduler')
  .withProps(Props.create(() => new JobScheduler()))
  .withRole('control-plane');   // optional: nur diese Nodes können hosten
