...
system.spawn(
  Props.create(() => new ClusterSingletonManager(clusterSingletonManagerOptions)),
...
Props.create(() => new ClusterSingletonManager(managerOptions));
```
...
<Aside type="caution" title="Manager-Name muss der Konvention folgen">
  ```ts
  system.spawn(Props.create(() => new ClusterSingletonManager(ClusterSingletonManagerOptions.create()/* ... */)), 'whatever');
  // ✗ Proxy sucht `/system/cluster/singleton/manager-<typeName>`
  ```
