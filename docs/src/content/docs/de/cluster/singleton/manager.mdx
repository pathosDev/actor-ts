...
<Aside type="caution" title="Eine Rolle, die niemand trägt, hostet nirgends">
  ```ts
  StartSingletonOptions.create<JobCommand>().withRole('control-plane');
  ```
  Der Manager hostet nur, wenn dieser Node **sowohl** Cluster-Leader ist
  **als auch** die Rolle trägt. Trägt der gewählte Leader sie nicht,
  hostet gar kein Node das Singleton — anders als Akka, das den ältesten
  Member *mit* der Rolle wählt. Gib die Rolle den Nodes, die zuerst
  sortieren, oder lass sie ungesetzt, bis das behoben ist.
</Aside>
