...
```ts
import { SingletonKey, StartSingletonOptions } from 'actor-ts';
...
`typeName`, `singletonProps` und die drei obigen — setzt die Extension
...
| `singletonProps` | Ja | Wie der Singleton konstruiert wird. Wird nur auf dem Host aufgerufen. |
