...
<Aside type="caution" title="Sie verweigert einen Wildcard-Host">
  ...
  leere String) ist eine *Bind*-Adresse: jeder Node, der darauf
  zurückgefallen ist, sortiert identisch, jeder hält sich für den
  Gewinner, und jeder bildet seinen eigenen Cluster. Die Phase lehnt das
  vorab ab. Setze `withHost(...)` explizit oder die Umgebungsvariable
  `CLUSTER_HOST` / `POD_IP`.
</Aside>
