...
### Wenn das Acquire hängt

Der interessante Fall ist nicht „die Lease sagt nein“, sondern
„die Lease sagt gar nichts“.  Wenn `acquire()` nicht innerhalb von
`acquireTimeoutMs` (Default 5 s) resolved, hört die Strategie auf
zu warten — und muss dann damit rechnen, dass der Versuch auf der
Leitung trotzdem noch erfolgreich ist und die Lease bei einem Node
beansprucht zurücklässt, der nicht mehr glaubt, gewonnen zu haben.
