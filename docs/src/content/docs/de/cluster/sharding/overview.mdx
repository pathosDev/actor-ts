...
nie einen Shard zugewiesen und hostet nie eine Entity, die
Fehlkonfiguration ist also ein Stillstand mit einem Fehler daneben
statt eines stillen Splits. Korrigiere den Wert und starte den Node
neu.

Das ist naturgemäß asynchron: Eine Region registriert sich, nachdem
`start(...)` zurückgekehrt ist, versucht es erneut, bis es einen
Koordinator gibt, und registriert sich bei jedem Leader-Wechsel neu. Es
gibt keinen Aufruf, aus dem geworfen werden könnte — deshalb kommt das
