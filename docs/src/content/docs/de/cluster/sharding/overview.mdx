...
type ShardingOptionsType<TMessage> = {
  ...
  maxEntities?:            number;            // Cap für residente Entities, pro Typ pro Node
  passivationReplacement?: EntityReplacementPolicy;  // welche Entity das Cap hergibt
};
```
...
| `maxEntities` | Wie viele Entities **dieses Typs** auf **diesem Node** resident sein dürfen. Wird das überschritten, wird eine passiviert, um Platz zu machen. `0` (Default) heißt kein Cap. Nicht node-weit: zwei gestartete Typen bekommen je die volle Zahl. |
| `passivationReplacement` | Welche residente Entity das Cap hergibt — `'least-recently-used'` (Default), `'segmented-least-recently-used'` oder `'least-frequently-used'`. Bei `maxEntities` `0` wirkungslos; siehe [Wählen, was ein voller Node hergibt](#wählen-was-ein-voller-node-hergibt). |
...
`hand-off-timeout` sowie der ganze `passivation`-Unterblock, der
entscheidet, welche Entity das Cap hergibt. Sie setzen die node-weite
Grundeinstellung für jeden sharded Typ und liegen *unter* dem, was
`start(...)` explizit übergibt — ein Per-Typ-Wert im Builder gewinnt also
weiterhin. Siehe
...
`maxEntities` überschritten wird (verdrängt wird die Entity, die die
Ersetzungsstrategie benennt), wird die Entity gestoppt. Es wird nichts an
die Entity geschickt, und es gibt keine Quittung. Die *Entscheidung*
trifft die Region, weil nur sie den Verkehr über alle Shards dieses Nodes
sieht — deshalb ist `maxEntities` eine Obergrenze pro Typ pro Node, nicht
pro Shard — und der Shard, dem die Entity gehört, führt sie aus.
...
Die Stop-Nachricht ist eine *Bitte*: die Entity entscheidet, wann sie darauf
reagiert. Tut sie es nie, terminiert sie nie — und ihr Platz bleibt für die
Lebensdauer des Nodes gegen `maxEntities` belegt. `passivationStopTimeoutMs`
(`actor-ts.sharding.passivation.stop-timeout`) ist die Rückfallebene dafür:
danach stoppt der Shard die Entity hart, genau so, wie es eine von der Region
getriebene Passivierung tut.

Sie ist **standardmäßig aus** (`0`, unbegrenztes Warten), und zwar weil der
kooperative Vertrag der Punkt ist. Eine Entity mitten im Leeren — ein langer
Flush, ein langsamer letzter Write — darf so lange brauchen, wie das Leeren
dauert, und ein harter Stopp bricht diesen Flush ab; nur du weißt, wie lange
deiner legitim laufen darf. Setze sie dort, wo eine Entity, die nie auf die
Nachricht reagiert, ein Fehler wäre, den es einzudämmen gilt. Sie ist von
`maxEntities` unabhängig: ein positiver Wert schärft sie mit oder ohne Cap.

### Wählen, was ein voller Node hergibt

`maxEntities` sagt, **wie viele** Entities resident bleiben;
`passivationReplacement` sagt, **welche geht**, wenn eine neue dazukommt. Jeder
Regler hier ist wirkungslos, solange `maxEntities` `0` ist — der Default.

Die Default-Strategie `'least-recently-used'` hat einen blinden Fleck:
Aktualität kann *einmal überhaupt benutzt* nicht von *bis eben ständig benutzt*
unterscheiden. Ein Durchlauf über einen großen kalten Schlüsselraum — ein
nächtlicher Export, ein Crawler, ein Backfill — verdrängt dein Arbeitsset also
Entity für Entity und hinterlässt einen Node voller IDs, nach denen nie wieder
jemand fragt. Zwei Optionen beheben das, und sie greifen ineinander:
...
const shardingOptions = StartShardingOptions.create<CartCommand>()
  ...
  // Bewährung für eine einmal gesehene Entity, geschützt für eine erneut
  // gesehene; verdrängt wird aus der Bewährung, ein Scan kann also nur einen
  // anderen Scan verdrängen.
  .withPassivationReplacement('segmented-least-recently-used')
  .withPassivationSegmentedProtectedProportion(0.8)
  // Ein kleines Fenster vor der Strategie, damit die bei der Aufnahme
  // beurteilte Entity nie die ist, auf die eine Nachricht wartet.
  .withPassivationAdmissionWindowProportion(0.01)
  // …und was beurteilt: ein kompakter Sketch darüber, wie oft jede ID gesehen
  // wurde — er erinnert sich auch an IDs, die der Node nicht mehr hält.
  .withPassivationAdmissionFilter('frequency-sketch');
```

`'least-frequently-used'` ist die dritte Strategie — die wenigsten Zugriffe
verlieren, wobei die Zähler periodisch halbiert werden, damit die Rangfolge dem
jüngeren Verkehr folgt statt einer Lebenszeitsumme. Sie passt zu einem
Arbeitsset, das stabil, aber nicht aktuell ist, etwa eine Geräteflotte, die in
langsamem Takt meldet.

Der Filter braucht ein Fenster: eine Region kann die Entity, für die gerade
eine Nachricht eingetroffen ist, nicht ablehnen — `'frequency-sketch'` mit
`passivationAdmissionWindowProportion: 0` wird deshalb bei `start(...)`
abgelehnt, statt still nichts zu tun. Alle fünf Regler sind zugleich
HOCON-Keys unter
[`actor-ts.sharding.passivation`](/de/reference/configuration/#actor-tssharding).
