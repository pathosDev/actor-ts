...
| Actor | Rolle |
| --- | --- |
| **Region** | Eine pro Node. Routet Nachrichten an den Besitzer des richtigen Shards und entscheidet, wann lokale Entities passiviert werden. |
| **Shard** | Einer pro hier gehostetem Shard, Kind der Region. Besitzt die Entity-Actors: spawnt sie, stoppt sie, puffert für eine, die gerade geht. |
...
Vier davon sind auch HOCON-Keys — `number-of-shards`,
`remember-entities`, `passivation-idle` und `max-entities` — ebenso die
Koordinator-Keys `rebalance-interval` und `hand-off-timeout`. Sie
setzen die node-weite Grundeinstellung für jeden sharded Typ und
liegen *unter* dem, was `start(...)` explizit übergibt — ein
Per-Typ-Wert im Builder gewinnt also weiterhin. Siehe
...
Es gibt **kein Standard-Idle-Fenster** — `passivationIdleMs` ist `0`
(deaktiviert), solange du es nicht setzt, im Code oder über
`actor-ts.sharding.passivation-idle`. Eine Entity, die niemand
passiviert, bleibt für die Lebensdauer ihres Nodes resident.
