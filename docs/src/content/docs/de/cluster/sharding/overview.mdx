...
Lebensdauer des Nodes gegen `maxEntities` belegt. Der Shard rüstet deshalb eine
Rückfallebene: nach `passivationStopTimeoutMs` (Default 10 Sekunden,
`actor-ts.sharding.passivation.stop-timeout`) stoppt er die Entity hart, genau
so, wie es eine von der Region getriebene Passivierung tut. Setze `0`, wo das
geordnete Herunterfahren tatsächlich keine Schranke hat.
