...
`maxEntities` überschritten wird (die LRU-Entity wird verdrängt),
stoppt die Region die Entity direkt. Es wird nichts an die Entity
geschickt, und es gibt keine Quittung.

**Manuell (graceful)** — eine Entity bittet ihre Region, sie zu
passivieren, indem sie ein `Passivate` an ihren Parent schickt.
`Passivate` trägt eine *Stop-Nachricht*, die die Region an die Entity
zurückleitet, sodass die Entity genau entscheidet, wann und wie sie
herunterfährt (laufende Arbeit abschließen, State flushen, dann
terminieren). `PoisonPill` als Stop-Nachricht terminiert die Entity,
sobald die Region sie zurückleitet:
