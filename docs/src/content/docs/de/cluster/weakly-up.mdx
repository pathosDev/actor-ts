...
await Cluster.join(
  system,
  clusterOptions,   // nach 3s in joining automatisch befördern
);
```
...
Die Verzögerung ist auch aus der Config setzbar — für ein
Deployment, das sie lieber pro Umgebung als pro Build einstellt:

```hocon
actor-ts.cluster.weakly-up-after = 3s   # 0 deaktiviert
```

`withWeaklyUpAfterMs(…)` gewinnt weiterhin, wo beides gesetzt ist
— die übliche Reihenfolge **explizite Optionen > HOCON >
eingebaute Defaults**.
...
<Aside type="caution" title="`minimumMembersBeforeUp` ist die andere Hälfte desselben Reglers">
  ```ts
  const clusterOptions = ClusterOptions.create()
    .withWeaklyUpAfterMs(3_000)
    .withMinimumMembersBeforeUp(3);
  ```
  Die beiden werden zusammen gelesen und wirken auf
  gegenüberliegende Seiten desselben Übergangs. `weaklyUpAfterMs`
  entscheidet, wie lange ein `joining`-Mitglied wartet, bis es eine
  *teilweise* Mitgliedschaft bekommt; `minimumMembersBeforeUp`
  entscheidet, ob der Leader überhaupt eine volle vergeben darf.
  Mit beiden gesetzt ist `weakly-up` genau der Zustand, in dem ein
  Node sitzt, solange die Mitgliederschwelle nicht erreicht ist —
  und ein `weakly-up`-Mitglied zählt weiterhin für diese Schwelle,
  die Beförderung rückt das Ziel also nicht weiter weg. Siehe
