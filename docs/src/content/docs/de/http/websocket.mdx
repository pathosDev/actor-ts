...
- **Im Transport**, bei **1 MiB** — dem eingebauten Default, den das
  Framework beim Binden an den Socket der Laufzeit weiterreicht.  Ein
  größerer Frame wird abgewiesen, *während er eintrifft*; ein feindlicher
  Peer kann den Prozess also nicht dazu bringen, 16 MiB (Bun) oder 100 MiB
  (`ws`) zu puffern, nur damit das Ergebnis anschließend verworfen wird.
  Weil die Laufzeit dabei auflegt, statt einen Policy-Close zu senden,
  sieht der Client hier einen abnormalen Close (**1006**) und keine 1009.
- **Im Connection-Actor**, bei deinem `maxFrameBytes` — am vollständig
  empfangenen Frame, vor dem Dekodieren, mit Close **1009**.

Wichtig dabei: Das Transport-Limit ist der Framework-Default, nicht die
Zahl deiner Route.  `maxFrameBytes` zu **senken** wirkt genau wie
geschrieben; es **über 1 MiB anzuheben, hebt das Transport-Limit nicht
mit** — Frames zwischen 1 MiB und deinem Limit werden weiterhin
abgeschnitten.  Backends sehen die Policy einer Route beim Binden nicht;
sie wird erst bei der ersten Verbindung aufgelöst.  Genau das muss ein
konfigurierbares Transport-Limit noch lösen.
