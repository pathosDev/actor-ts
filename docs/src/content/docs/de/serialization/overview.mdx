...
<Aside type="caution" title="Einen laufenden Cluster aktualisieren">
  ...
  **breaking Wire-Änderung**.  Ein neuerer Node liest die Frames
  eines älteren unverändert — untaggtes JSON dekodiert als es
  selbst —, umgekehrt aber nicht: ein älterer Node sieht den
  Tag-Wrapper als einfache Daten.  Betroffen sind nur Payloads, die
  ohnehin schon beschädigt wurden; ein Cluster, dessen Nachrichten
  einfache Daten sind, kann Node für Node rollen, einer, der eine
  `Map`, `Set`, `Date`, `bigint` oder ein Typed Array sendet,
  sollte in einem einzigen Fenster aktualisieren.
</Aside>
