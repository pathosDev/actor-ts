...
```ts
class VoiceSessionActor extends PersistentActor<SessionCommand, SessionEvent, SessionState> {
  ...
  async onCommand(state, command: SessionCommand) {
    if (command.kind === 'audio-chunk') {
      const text = await transcribe(command.audioBytes);
...
```ts
const byTagProjectionOptions = ByTagProjectionOptions.create<SessionEvent>()
  .withName('transcript-search')
  .withTag('transcript')
  .withQuery(new CassandraQuery({ /* ... */ }))
  .withOffsetStore(new InMemoryOffsetStore()) // oder DurableStateOffsetStore(store) für neustartsichere Offsets
...
const projection = ProjectionActor.byTag<SessionEvent>(system, byTagProjectionOptions);
