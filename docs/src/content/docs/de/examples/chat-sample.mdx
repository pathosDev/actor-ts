...
# Das Demo-Secret für Session-Tokens für einen lokalen Lauf akzeptieren (siehe unten):
export CHAT_ALLOW_DEMO_SECRET=1
...
Die eine Variable ist Absicht. Session-Tokens sind HMAC-signiert und
validieren sich selbst, das Signier-Secret ist also das Einzige, was
zwischen einem Fremden und einer beliebigen Identität steht — und ein
Sample, das ein fehlendes Secret durch eine im eigenen Quelltext
veröffentlichte Konstante ersetzt, käme gesund aussehend hoch,
während jeder, der das Repository gelesen hat, ein Token für jeden
beliebigen Benutzer ausstellen könnte. Deshalb wirft der Store
stattdessen, und `CHAT_ALLOW_DEMO_SECRET=1` ist die Art, wie eine
lokale Demo sagt, dass sie diese Konstante bewusst akzeptiert. Alles
jenseits eines Laptops exportiert stattdessen `CHAT_TOKEN_SECRET` —
auf jedem Node denselben starken Zufallswert, sonst bricht Resume
beim Fail-over.
...
# that node needs a token secret like any other:
CHAT_ALLOW_DEMO_SECRET=1 bun examples/chat/smoke-test.ts --spawn-backend
```
