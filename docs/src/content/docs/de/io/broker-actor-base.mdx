...
override onReceive(message: MyCommand): void {
  ...
  // Command-Union — deshalb wird der *Match-Input* geweitet, siehe unten.
...
<Aside type="caution" title="Der Terminated-Arm ist nicht optional">
  ...
  Defekt (#1111), kein hypothetischer — und der `.exhaustive()`-Aufruf
  *wirft* auf dem ungematchten Signal, was der Standard-Supervisor mit einem
  Restart beantwortet, also mit einem vollen Broker-Reconnect pro
  Subscriber-Tod.

  Schreib es genau wie oben, dann kann beides nicht passieren: die Weitung des
  **Match-Inputs** auf `MyCommand | Terminated` lässt `.exhaustive()` ohne den
  Arm nicht mehr kompilieren (`NonExhaustiveError<Terminated>`) — das Weglassen
  ist damit ein Build-Fehler und kein Laufzeitfehler.  Der *Parameter* bleibt
  `MyCommand`, denn `Actor<Command>` legt den Mailbox-Typ fest, und die Weitung
  ist tragend statt kosmetisch: `P.instanceOf(Terminated)` typcheckt gegen eine
  ungeweitete Union nicht.
...
<Aside type="caution" title="Die Death-Watch feuert nur für einen lokalen Subscriber">
  ...
  `tell` über die Node-Grenze.  Der Arm oben kann daran nichts ändern — das
  Signal kommt nie an.  Nachverfolgt als #918.
</Aside>
