...
```ts
import { ActorSystem, Actor, Props, JetStreamActor, JetStreamOptions } from 'actor-ts';
...
```ts
type JetStreamOptionsType = BrokerCommonOptionsType & {
...
<Aside type="caution" title="KV und Object Store sind separat">
  JetStreams Key-Value- und Object-Store-Sub-APIs haben andere Semantik
  und werden von diesem Actor nicht abgedeckt
  ([Roadmap #74](https://github.com/pathosDev/actor-ts/issues/74)).
