...
- **Lifecycle ist unabhängig.**  Stoppe den Router → Routees laufen
  ...
  niemand hat, mit einer Warnung.  Der Rest des Pools ist unberührt.
...
| Einen Routee | Gecrasht → an Ort und Stelle vom Supervisor neu gestartet (Default-Strategie). Gestoppt → auf sein `Terminated` entfernt: Der Pool schrumpft um einen, und in die Lücke wird nichts geroutet (#449). | Router sendet weiter an diesen Node, bis sich dessen Mitgliedschaft ändert — er prüft Routees nicht auf Lebendigkeit —, und der Node verwirft diesen Anteil mit einer Warnung; Geschwister unberührt. |
