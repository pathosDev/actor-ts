---
title: Pool vs Group
description: Zwei Wege, wie Router Routees erwerben — **Pool**-Router spawnen ihre Routees aus einer Actor-Klasse oder Factory (der lokale Router); **Group**-Router finden existierende Routees per Pfad (ClusterRouter). Der Trade-off in Lifecycle-Besitz und welche Routees jedes passt.
---
...
| --- | --- | --- |
| **Pool** | Der Router — er spawnt die Routees selbst. | Statisch zur Spawn-Zeit; Größe ändert sich nicht ohne Router-Restart. |
| **Group** | Jemand anderes — die Routees existieren bereits. | Dynamisch (Group-Router re-discovern bei Bedarf). |

Der lokale `Router` ist **immer ein Pool** — einschließlich
[`Router.scatterGatherFirstCompleted`](/de/routing/scatter-gather/),
das seine Routees wie jede andere lokale Factory selbst spawnt und
besitzt.  Der Cluster-`ClusterRouter` ist **immer eine Group** (Routees
werden aus Cluster-Up-Members an einem bekannten Pfad abgeleitet).  Zu
...
- **Supervision ist unkompliziert.**  Der Router supervised seine
  Routees mit der Strategie, die die Routee-Options deklarieren
  (oder dem Default).
...
Richtige Form für **homogene Worker-Pools** — N zustandslose Workers,
alle gleich, die einen Stream I/O-gebundener Arbeit am Laufen halten.
Das ist Nebenläufigkeit auf dem Thread des Routers, keine
CPU-Parallelität; für den Pool, der Threads spannt, siehe
[Eine Adresse, ein Actor pro Thread](/de/routing/overview/#eine-adresse-ein-actor-pro-thread).
...
- **Lifecycle ist unabhängig.**  Stoppe den Router → Routees laufen
  weiter.  Stoppe einen Routee → der Router bemerkt es nicht: Routees
  werden aus der *Mitgliedschaft* abgeleitet, nicht auf Lebendigkeit
  geprüft, er sendet diesem Node also weiter seinen Anteil, bis der Node
  selbst geht — und der Node verwirft, was für einen Pfad ankommt, den
  niemand hat, mit einer Warnung (der des Clusters selbst, `no envelope
  handler registered`, oder, auf einem Node, der `ClusterSharding`
  gestartet hat, der der Sharding-Extension, `[sharding] no
  region/coordinator registered for …` — sie übernimmt auf diesem Node
  jeden nicht aufgelösten Envelope).  Der Rest des Pools ist unberührt.
...
Visier nimmt.  Auf dem Worker-Mesh ist diese Komposition ein
dokumentiertes Rezept — das `setup` des Actor-Moduls spawnt den Routee
auf jedem Worker, und ein `ClusterRouter` auf dem Main-Thread findet
sie per Pfad und Rolle; siehe
[Eine Adresse, ein Actor pro Thread](/de/routing/overview/#eine-adresse-ein-actor-pro-thread).
Für einen Cluster aus Prozessen siehe
...
| Einen Routee | Gecrasht → an Ort und Stelle vom Supervisor neu gestartet (Default-Strategie). Gestoppt → auf sein `Terminated` entfernt: Der Pool schrumpft um einen, und in die Lücke wird nichts geroutet (#449). | Router sendet weiter an diesen Node, bis sich dessen Mitgliedschaft ändert — er prüft Routees nicht auf Lebendigkeit —, und der Node verwirft diesen Anteil mit einer Warnung (der des Clusters, oder der der Sharding-Extension auf einem Node, der `ClusterSharding` gestartet hat); Geschwister unberührt. |
...
- **[Router](/de/routing/router/)** — die lokale
  Pool-Router-API.
- **[Scatter/Gather](/de/routing/scatter-gather/)** — der
  antwort-förmige Pool-Router, aus denselben Gründen nur Pool.
