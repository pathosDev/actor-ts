...
| --- | --- | --- |
| Fester Sleep vor einer Assertion | Wird umgestellt | `await sleep(N)`, dann `expect(...)` auf Zustand, den ein Hintergrundschritt erzeugt |
...
**Diese Umstellung ist nicht abgeschlossen.**  Unter `tests/` stehen weiterhin
466 `await sleep(`-Stellen gegen 643 `awaitCondition(`-Stellen — und die
Sleep-Zahl ist während der Umstellung *gestiegen*, weil neue Tests weiter in
der alten Form geschrieben werden.  Ein zweiter Helper trübt das Bild
zusätzlich: `waitFor` taucht 332-mal auf und wird in 32 einzelnen Dateien als
lokales `function waitFor(` neu deklariert, jedes Mal mit eigenem Timeout und
Poll-Intervall.  Behandle ein `sleep`-dann-`expect`-Paar in einem umgefallenen
Test als Verdächtigen, bevor du nach etwas Raffinierterem suchst.

Diese vier Zahlen sind `grep -ro '<pattern>' tests/ --include=*.ts | wc -l`
(die letzte `grep -rl`), also nachmessbar statt zu glauben; zuletzt erhoben am
16.08.2026.  Sie stehen aus demselben Grund exakt da: eine gerundete Zahl
driftet unbemerkt, eine exakte ist sichtbar veraltet, sobald sie nicht mehr
stimmt.
