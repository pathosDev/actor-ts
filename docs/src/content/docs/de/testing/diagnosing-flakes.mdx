...
- **Zwei hatten nie eines.**  `peers list is empty after shutdown` ist von Anfang
  ...
  Verzögerung, also bleibt als Verdächtiger die modulweite Registry, die
  `InMemoryTransport` zwischen Tests teilt.  `ParallelMultiNodeSpec.test.ts`
  enthält **überhaupt** kein Warten auf eine feste Verzögerung; es wartet über
  die Budgets von `spec.awaitMembers` / `spec.awaitMemberStatus`.
