---
title: Supply Chain
description: Was die Release-Pipeline über eine veröffentlichte actor-ts-Version garantiert — npm-Provenance, eine CycloneDX-SBOM ab dem nächsten Release, CodeQL-Analyse und ein Advisory-Gate über die echte Lockfile — und wie du jeden Punkt selbst nachprüfst.
---
...
<Aside type="note" title="Die SBOM beginnt mit dem nächsten Release">
Die beiden Jobs, die sie bauen und anhängen, sind *nach* der
Veröffentlichung von v0.16.0 in `publish.yml` gelandet — kein Release,
das du heute herunterladen kannst, trägt das Asset also:
`gh release view v0.16.0 --json assets` antwortet mit einer leeren Liste.
Das erste Release aus der aktuellen Pipeline ist das erste, das sie
mitbringt. Die anderen beiden Zeilen gelten für jedes bereits
