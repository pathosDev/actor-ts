...
import { DistributedDataId, GCounter } from 'actor-ts/crdt';
...
The CRDT-type deep dives cover `merge()` semantics, when they fit,
and when they don't —
[counters](/distributed-data/crdt-types/counters/),
[registers](/distributed-data/crdt-types/registers/),
[sets](/distributed-data/crdt-types/sets/) and
...
| `{ from: 3 }` | Wait for exactly `from` acks (clamped to `[1, N]`). |
...
```ts
import { DistributedDataOptions } from 'actor-ts/crdt';
...
`withDurableStore` takes any [`DurableStateStore`](/persistence/durable-state/)
(InMemory here; a SQL or object-storage backend in production).  The
durable layer persists the replica's **whole view** to the store; on
restart, the state is restored before joining the gossip.
...
- **[Counters](/distributed-data/crdt-types/counters/)** — the first
  of the CRDT-type deep dives; each one links on to registers, sets
  and maps.
