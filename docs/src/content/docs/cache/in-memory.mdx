...
## Eviction is blind to what an entry protects

Recency is the *only* criterion.  A rate-limit counter and an
idempotency record are evicted exactly as readily as a cached response
body — and dropping one silently voids the guarantee it carried:
...
Neither leaves a trace: the entry is simply gone, well inside its TTL.

The exposure comes from **sharing one cache instance** between a
consumer that carries a guarantee and one whose key space a caller can
enumerate.  A response cache keyed on a path parameter and an
`idempotent` wrapper keyed on a client-supplied header are both in the
second category, so a caller who varies either one turns the whole map
over.  With the default 10 000-entry bound and a permissive limit of
100 requests per second, that takes about 100 seconds — entirely
*inside* the documented rate limit.

The fix is one cache per consumer:
...
reach the others.  Size each one's `maxEntries` for that consumer's key
space alone.

This narrows the blast radius rather than removing it: a middleware
whose own key space is attacker-controlled can still evict its own
entries — a client with an IPv6 `/64` mints rate-limit keys all day.
Where that matters, back the security-relevant consumer with Redis
(which you size and monitor independently) instead of an in-process
LRU.
