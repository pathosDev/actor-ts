...
Recency is not the *only* criterion, though: which entries the cap
reaches for first depends on what they carry, and that is the next
section.
...
- **The policy does not rank guarantees against each other.**  Two
  ...
  other, so one cache per consumer stays the advice (below).
...
name is yours, so these paths are not listed in
...
That narrows the blast radius rather than removing it: a middleware
whose own key space is attacker-controlled can still evict its own
entries — a client with an IPv6 `/64` mints rate-limit keys all day, and
every one of them is a counter this policy protects.  Where that
matters, back the security-relevant consumer with Redis (which you size
