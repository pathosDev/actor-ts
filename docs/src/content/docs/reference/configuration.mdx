...
| `http.shutdown-grace-period` | `5s` | How long `unbind()` lets in-flight requests drain before connections are forced. |
...
The grace period is an **upper bound, not a delay**: `unbind()` returns as
soon as the server has actually closed, and only waits out the window if
requests are still in flight.  A caller that passes its own value —
`binding.unbind(1_000)` — overrides it.
