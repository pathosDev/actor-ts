...
| `http.websocket` | *(see below)* | Server-side WebSocket defaults for [`websocket()`](/io/server-websocket/) routes. |
...
- For **broker actors** (MQTT, Kafka, WebSocket client, …) the error is
  ...
  `BrokerSettingsError`, and only *present-but-invalid* values become
  `OptionsError`.
...
type, missing path) and `BrokerSettingsError` (a required broker setting
