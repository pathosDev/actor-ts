...
| Key | Default | Purpose |
| --- | --- | --- |
| `dead-letters.store` | `"off"` | `off` = publish only, keep nothing / `memory` = a bounded ring / `persistent` = also write to the journal, so the queue survives a restart. |
| `dead-letters.max-entries` | `1000` | Letters held before the oldest is evicted. |
...
See [Dead letters](/operations/dead-letters/) for what the queue holds and
how to replay from it.
