...
Both caps exist because the replicator's mailbox already has one, and the
mailbox's cap fails badly here.  The replicator runs on the default bounded
mailbox (10 000 entries, `drop-head`), so at saturation the *oldest* queued
envelope is discarded — and a queued `ddata-update` carries the caller's
`resolve` / `reject`, so dropping it leaves `updateAsync` awaiting a promise
that can never settle, with nothing logged.  Set well below the mailbox's
capacity, `max-pending-quorum-requests` turns that silent drop into an
explicit rejection naming the knob; set *at* 10 000 it would never fire
first and would convert nothing.  See
