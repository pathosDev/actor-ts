...
```mermaid
flowchart TB
  EVT[/"LeaderChanged event fires"/]
  LEADER{"is this node<br/>now leader?"}
  ACQ["lease.acquire"]
  GOT{"acquired?"}
  SPAWN["spawn singleton<br/>register onLost"]
  RETRY["retry after<br/>acquireRetryDelay"]
  STOP["stop the singleton<br/>lease.release"]
  LOST[/"onLost fires"/]
  REACT["stop singleton<br/>await next LeaderChanged"]
  EVT --> LEADER
  LEADER -- "yes" --> ACQ --> GOT
  GOT -- "true" --> SPAWN
  GOT -- "false" --> RETRY
  RETRY --> ACQ
  LEADER -- "no, lost it" --> STOP
  LOST --> REACT
...
2. **`lease.checkAlive`** before issuing each allocation.
3. **`onLost`** → reject pending allocations, stop coordinator.
