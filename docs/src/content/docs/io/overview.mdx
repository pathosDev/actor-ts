...
<Aside type="caution" title="HTTP servers are a separate module">
  The framework has a dedicated [HTTP](/actor-ts/http/overview/)
...
- **Per-protocol pages** — [Kafka](/actor-ts/io/kafka/),
  [MQTT](/actor-ts/io/mqtt/), [AMQP](/actor-ts/io/amqp/),
  [NATS](/actor-ts/io/nats/), [Redis Streams](/actor-ts/io/redis-streams/),
  [gRPC](/actor-ts/io/grpc/), [WebSocket client](/actor-ts/io/websocket/),
  [WebSocket server](/actor-ts/io/server-websocket/),
  [SSE](/actor-ts/io/sse/), [TCP](/actor-ts/io/tcp/),
  [UDP](/actor-ts/io/udp/).
- **[BrokerActor base class](/actor-ts/io/broker-actor-base/)** —
  the shared lifecycle and configuration.
- **[HTTP overview](/actor-ts/http/overview/)** — the separate
  HTTP server module.
- **[Event stream](/actor-ts/fundamentals/event-stream/)** — where
  broker lifecycle events are published.

The [`BrokerActor`](/actor-ts/api/classes/brokeractor/) API
