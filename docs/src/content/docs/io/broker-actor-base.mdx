...
- **Message handling** (`onCommand`) — one command from the mailbox.
  This is where a subclass's `match(...).exhaustive()` dispatch table
  lives.
...
<Aside type="tip" title="Removal on death is the base class's job">
  Nothing above mentions `Terminated`, and nothing needs to: `onReceive` is
  sealed, so the base class catches the signal, drops the subscriber from
  every topic it held, and hands `onCommand` only real commands.  Your
  dispatch table is over your own protocol and `.exhaustive()` is exactly
  right on it.

  This used to be the subclass's job, and getting it wrong was easy in both
  directions: a matcher without a `Terminated` arm *throws* on the unmatched
  signal, which the default supervisor answers with a restart — a full broker
  reconnect per subscriber death, until `maxRetries: 10` stops the bridge for
  good — and one that swallowed it with `.otherwise()` leaked instead, the
  dead ref staying in every topic and costing a dead-lettered `tell` on each
  fan-out (#1111, #709).

  `pruneTerminatedSubscriber(ref)` is still there, for a subclass that stops
  a subscriber deliberately rather than waiting for the watch.  It returns
  whether it removed anything, and does **not** `unwatch`: on the death path
  the cell has already dropped the watch by the time the signal arrives.
</Aside>

<Aside type="note" title="Watching refs of your own">
  Override `onTerminated(signal)` if your actor calls `context.watch` for
  something other than a subscriber — a delivery target, a request's
  originator.  It runs **after** the base has pruned, so the subscriber
  registry it sees has already forgotten the dead ref.  `MqttActor` uses it
  for its per-pattern delivery targets.
</Aside>
...
<Aside type="caution" title="The watch only fires for a local subscriber">
  ...
  stops, and keeps costing a cross-node `tell` on each fan-out.  No
  `Terminated`-based cleanup can help — the signal never arrives.  Tracked
  as #918.
</Aside>
