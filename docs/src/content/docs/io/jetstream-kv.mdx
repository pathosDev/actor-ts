...
# or: bun add @nats-io/transport-node @nats-io/jetstream @nats-io/kv @nats-io/obj
```

Four packages, and that is the change worth noticing: the key-value and
object-store views used to ship inside the same package as
[NATS](/io/nats/) and [JetStream](/io/jetstream/), and nats.js v3 gave each
its own. Drop `@nats-io/obj` if you only need key-value, or `@nats-io/kv` if
you only need the object store.
