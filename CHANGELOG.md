...
- **`CborSerializer` carries the same rich types as the JSON tree** (#1036).
  `Map`, `Set` and `BidirectionalMap` used to fall into the CBOR encoder's
  generic object branch, where `Object.entries` is `[]` for all three: they
  encoded as an empty `{}` and every entry was lost, with nothing raised.
  ...
  suite rather than shipping as silent data loss.
