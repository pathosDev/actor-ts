...
export function App(): React.JSX.Element {
  ...
  switch (v.state.phase) {
    case 'gate-mic':   return <GateMicView v={v} />;
    case 'gate-login': return <GateLoginView v={v} />;
    case 'app':        return <AppView v={v} />;
  }
}

function GateMicView({ v }: { v: VoiceHandle }): React.JSX.Element {
  return (
    <div className="gate">
...
function GateLoginView({ v }: { v: VoiceHandle }): React.JSX.Element {
                    ...
                    {entered ? (
                      <>
                        <button className={'talk' + (talking ? ' active' : '')}
                                onClick={() => v.toggleRoomTalk(r)}>{talking ? 'Stop' : 'Talk'}</button>
