...
type DomainEvent = { readonly type: string; readonly payload: unknown; };
...
async function main(): Promise<void> {
  const nodeA = await startNode('a', 7100);
  const nodeB = await startNode('b', 7101, ['events@a:7100']);
  const nodeC = await startNode('c', 7102, ['events@a:7100']);
