...
import { LokiSink } from './LokiSink.js';
import { isLokiSinkEnabled, readLokiSinkOptionsFromConfig } from './LokiSinkOptions.js';
...
import { isSyslogSinkEnabled, readSyslogSinkOptionsFromConfig } from './SyslogSinkOptions.js';
...
export function buildLoggerFromConfig(
  ...
  if (isLokiSinkEnabled(config)) {
    sinks.push(new LokiSink(readLokiSinkOptionsFromConfig(config)));
  }
  ...
  if (isSyslogSinkEnabled(config)) {
    sinks.push(new SyslogSink(readSyslogSinkOptionsFromConfig(config)));
  }
